...
> Last updated: 2026-09-23（コンテナ形式の波 段2完了・次は段3 — 段1完了・ADR 0109〈manifest karume/5〉accepted・hub / runtime / exporter / modelsの2a〜2e済・ローカルミラー11本を`karume/5`へ移行済み・2f〈RAMピークharness `tools/ram-peak/matrix.ts`・検収②③は[研究記録](../docs/research/2026-09-23-container-ram-peak.md)〉も済・irodori-v4.1-smallの再アップロードとpin更新も済（検収①の実pin）。backlogの消化済み12節は[退避スナップショット](../docs/research/2026-09-22-backlog-archive-0.5.0-to-0.12.0.md)へ移し、backlogは運用契約どおり未消化項目だけになった）

## 現在の焦点

- コンテナ形式の波（2026-09-22〜・段2完了・次は段3）: 配布形をsafetensors方言から専用コンテナへ移す。**段2で入ったもの**（正本は[ADR 0109](../docs/decisions/0109-manifest-v5-container.md)・[ADR 0108追記2 / 3](../docs/decisions/0108-container-format.md)）: manifest `karume/5`（`weights.<部品>.<dtype>.container = { descriptor: 2文書の期待値, parts: part 0を含む全partのFileRef }`・quant席は写像のまま・model単位の`assets`は残す・hubは`karume/5`だけを読む）/ hubの`resolveSelection` / `selectionRefs` / `openContainerSource`（温めなし・区間読みだけ・seek / scanで分岐・HF経由は`verified`）/ runtimeの`BlockSource.verified`（検証済みならblockのsha256を掛けない）・`OpenedContainer.asset(name)`・`assets[].length` / modelsの継ぎ目`hub/components.ts`（descriptor → admission → 重みのprefetch → 資産の順・部品差し替え席`fromPretrained(source, { components })`・`fromAssets`は単一形krmかpart列）/ PLEは容器の資産（索引schema 3・values / scalesのblock列・1 block = 1 part・既定の常駐上限は最大block 2本 ≈ 64 MiB）/ exporterの`karume migrate --manifest`（リポ丸ごと・PLE・越境の引き写し）と`karume verify --container`。**落とし穴**: `models/`は移行済み（`karume/5`）で旧ミラーはリポ外`~/workspace/karume-models-v4/`（git追跡外）・HFのpinはirodori-v4.1-smallだけ更新済みで、残りは段3の再アップロードまで旧版パッケージからしか動かない・全量面`assetComponentOpener`は全部品を先に開く（admissionより先）・`ModelComponent.asset(name)`は呼ぶたびに新しい読み口で未検証の取得元ではblockを寿命ぶん保持する（1回の読みより長く握らない）・部品差し替えはグラフ記述のsha256が同一の部品だけ（QATはcheckpointごとに違うので拒否）。**段1で入ったもの**: `docs/ir-v2.md`（IR v2 + 正準直列化・initializer名 = テンソルキー）/ `packages/runtime/src/format/container/`（limits・codecs台帳・header・json・descriptor・open・bind）/ ランタイムの格納語彙は合流後の1本（`storage.codec` / `rowAxis` / `groupSize`・旧v1ローダは読んだ時点で改名 + 写像・旧形式の読みは段3で削除）/ `prepareContainer` + `createSessionFromContainer`（errorScopeはblockごと・フェンスはpartごと）/ Pythonの`karume.container`（writer + reader）と`karume migrate`。**落とし穴**: `IrGraph.initializers[*]`に`tensor`は無い（名前が鍵）・shared宣言は`{shared:true}`だけで格納を持たない（期待席は貸し手のcodecから構築時に導く）・`ReadyInitializer`はバイト列（scaleはrank 2 `[rows, groups]`）・v1 JSONを組むテストは入力側だけ旧綴りのまま。正本は[ADR 0108](../docs/decisions/0108-container-format.md)と[container-v1](../docs/container-v1.md)で、どちらも**accepted**（2026-09-22 に段1着手を裁定・codec登録名4種も確定）。`krm`=モデル（グラフ+重み+scale+資産）/ `krg`=グラフ（**重みblockを持たない**もの）で、種別を持つのはmagic 4バイトだけ。descriptorは**グラフ記述とモデル記述の2文書**に割り、`krg`はヘッダだけ書き換えたバイトコピーで抜ける（同一性は名前でも「同アーキ」でもなく内容ハッシュで判定する）。物理はblock（取得・検証・解放の単位・**32 MiB以下**・末尾paddingは書き手が焼く）とpart（配信粒度・`{256, 512, 768, 1024} MiB`から書き手が選ぶ・既定256 MiB）の2層で、part 0=ヘッダ+descriptor・part 1=const領域・part 2以降が重み/資産。格納の宣言は**codec台帳**（初版4種 = `int8-sym` / `int4-sym-g` / `int2-off` / `ternary`）へ寄せ、bit数は`packing`からの派生値にする。**旧safetensors形式との互換は全部切る**（両読みは実装せず、旧形式を読むのは移行CLIだけ。旧pinを持つ旧版パッケージは動き続ける）。この形にする動機は無改変の上流重みの読み込み・**部品単位の差し替え**（`fromPretrained`の`components`席・検査はadmissionに置き「重みを1バイトも取る前」を保つ）・**LoRAの実行時A/B**（段5）の3つ。段0の残りの宿題は`pushErrorScope('validation')`の同期区間をblock単位へ割る費用で、**計測中**（結果はADR 0108へ追記する）。段1〜6の作るものと検収はADR 0108の段階分解の表、作業順は[backlog](../docs/backlog.md)のnow先頭が正本。用語は[glossary](../docs/glossary.md)の「配布コンテナ」節。
