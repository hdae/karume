...
- **性能波（2026-08-13・設計変更不要枠）— 消化完了**: 第 1 段の内訳再実測
  ...
  拡充 → ④リリース準備）。**波① 済（同日）**: K-4a = conv1d implicit GEMM（ADR 0053・
  `7a725d7`・conv1d 16.0 倍・irodori 全 GPU −27%・WAV sha 不変）+ K-7 実測 → **棄却裁定**
  （`07b0be3` — H-2 実測でホスト利得 ≈8ms と確定・均一按分の 50 分の 1）。**波② 済
  （同日・ADR 0054）**: H-2 分解（フェンス待ちが支配・床 ≈11ms/本 — research/
  2026-08-13-host-cost-decomposition.md）→ H-5 = resident tensor + batch enqueue で
  ...
  - H-1 = 通常 run の単一フェンス化（`e339cc0`）。**検収: irodori 壁 8.59 → 4.88s
    ...
    後段 K-5b は**スパイク棄却 = K-5 クローズ**（2026-08-14・案 X〈鎖融合・ビット同一〉が
    S=750 ×1.19 で kill 基準 1.3 未満・S=170 は逆転 —
    [research/2026-08-14-attention-chain-fusion-spike.md](../docs/research/2026-08-14-attention-chain-fusion-spike.md)・
    online は保留 = LLM 波で attention op 側の席として再評価）。
    **次 = 整理整頓波（ユーザー裁定 2026-08-14 — LLM/拡張波の前に資料・コードの整理を優先・
    K-2 はその後）**。
