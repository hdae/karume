...
> Last updated: 2026-09-21（コード品質管理の波 段 1 済 — 段 2 / 3 は backlog now の同名項）

## 現在の焦点

- コード品質管理の波（2026-09-21〜・段1 済）: 入力は外部レビューの分割候補（triage.md §6・3段）。段1で共通層が増えた — `models/src/config/readers.ts`（pipelineConfigの基本reader）・`models/src/session/with-session.ts`・`models/src/text/{asset-gates,code-ranges}.ts`（家族横断の門と区間探索）。分割の規律: 数式・await・受理集合・文言を移動と同じ変更に混ぜない、WGSL生成器はスナップショットのバイト同一で確認、公開面fixtureは自動更新しない。
