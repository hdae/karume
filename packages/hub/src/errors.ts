...
/**
 * hub が**公開する**エラー型の基底（`instanceof HubError` で一括して捌けるように公開する）。
 * 公開するのは「利用者の入力・環境に起因する失敗」だけ（`mod.ts` の方針）なので、取得ではない面
 * — `openAsset` が返す区間読み — の失敗は取得元の素の `Error` のまま上がり、この基底を通らない。
 *
