...
//     ままなので、この門が要る
//  ③ **PLE が全量常駐しない**（ADR 0085 決定 3）: 索引が指す block は `model` 容器の資産として
//     遅延側で受け、触った 1 本だけを読む。全量を `fetchAssets` へ流す形へ戻ると 2.27GiB が常駐する
//  ④ **配布形の宣言と現物の対応**: 索引が名指しする block が `model` 容器の資産として全件そのまま
//     在ること（取得キーを「並び順で合わせる」形に戻っていないこと）
//
// ## 資産と経路
//
