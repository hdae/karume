/**
 * 決定的な標準正規乱数（初期 latent に使う）。
 *
 * 乱数はグラフに焼かない（ADR 0013 — 実行時ノブと乱数はホスト側）。パイプラインが seed 付きの
 * 生成器を持つのは、**同じ seed なら同じ画像が出る**ことを再現性の前提にするため。
 *
 * ## MUST: torch の `randn` とは別物である
 *
 * torch は Philox / Mersenne Twister（`torch.manual_seed`）で、こちらは splitmix64 +
 * Box–Muller。**同じ seed を与えても同じ列にはならない**ので、参照フィクスチャとの突合を
 * この生成器で作った latent でやってはいけない（分布が同じでも列が違うので、絵は別物になる）。
