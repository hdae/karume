 ...
 * モデルの入力の作り方を知ってはいけない（ADR 0008 の公開面の薄さ）。**利用者はデモ
 * （examples/sbv2）と検証（packages/models/tests/）の 2 つ**で、正本はモデル側の知識を持つ
 * models パッケージに置く（両者ともここを直接 import する — 実装を写した薄い面は作らない）。
 *
 * ## なぜホストが表を作るのか
 *
