 ...
 * - MUST: 最後の積和は **`fma`**。素の `a * b + c` はドライバが融合する**かもしれない**形で
 *   実装依存になる（本機 RTX 3080 Ti では実測で融合された — 素の式・`let` 経由・`bitcast`
 *   固定の 3 形とも同じ融合値）。ただし WGSL 仕様は `fma` の精度を `x*y+z` から**継承**と
 *   規定するのみで（§15.7.4.1 — 非融合展開も適合）、単一丸めは**仕様保証ではなく実装性質**。
 *   TS 参照（倍精度で 1 度だけ丸める）とビット一致するのは融合実装で、非融合実装では
 *   1 ULP 帯の差になり得る（Metal の実測 = docs/known-issues.md）。
 ...
 * ```
 *
 * - 丸めは **fma が `k/g + 1` 回**で、flush は **group 境界ちょうど**（K タイル境界ではない）。
 *   この勘定は fma を単一丸めと数え、`f32(acci)` の変換が無損失な `g ≤ 16,513` の範囲に限る
 *   （門 {@link LINEAR_W4A8_MAX_GROUP} はその外側も通す — ADR 0076 §2 と追記 2026-08-31）。
...
/**
 * i32 縮約のオーバフロー門。活性は quantize_rows が ±127 に閉じるが、**重みは i8 の
 * −128 を含みうる**ので上界は `|acc| ≤ k · 127 · 128 = k · 16256` — i32 の正側上限
 * 2,147,483,647 からは `k ≤ 132,104`（±127 前提の従来導出 k ≤ 133,144 は 2 だけ緩い）。
 * 門の 2^17 = 131,072 はどちらの上界の内側でもあり（131,072 · 16256 = 2,130,706,432 <
 * 2^31）、DiT の最大 k = 8,192 に対し余裕 16 倍。超える k は fail loudly — 黙って通すと
 * i32 の巻き戻りで符号ごと化ける。
 */
