...
/**
 * グラフ shard を読んで validator を組む。旧配布形の読み手は合流後のグラフと**付随情報**
 * （`legacy.scaleKeys` = companion scale の safetensors キー）を対で返し、validator は両方を要る。
 */
const validatorFor = (graph: GraphJson, tensors: readonly TensorSpec[] = []): ShardValidator => {
  const opened = extractIrGraph(graphShard(graph, tensors));
  return createShardValidator(opened.graph, opened.legacy);
};
...
Deno.test("extractIrGraph: グラフ shard から IR を取り出す", () => {
  const { graph, legacy } = extractIrGraph(graphShard(baseGraph(), [B_F32]));
  ...
  // 旧 shard の付随情報（companion scale のキー）— f32 だけのグラフには 1 本も無い
  assertEquals([...legacy.scaleKeys], []);
});
...
Deno.test("createShardValidator: グラフ shard + 重み shard を逐次受理して読了できる", () => {
  const validator = validatorFor(baseGraph(), [B_F32]);

  // グラフ shard に載った小テンソルもこの経路で実体が確定する（別扱いにしない）
  const fromGraphShard = validator.intake(graphShard(baseGraph(), [B_F32]));
  assertEquals(fromGraphShard.map((ready) => ready.name), ["enc.b"]);
  assertEquals(fromGraphShard[0].payload, B_F32.data);
  assertEquals(fromGraphShard[0].scale, undefined);

  const fromWeightShard = validator.intake(weightShard([W_F32]));
  assertEquals(fromWeightShard.map((ready) => ready.name), ["enc.w"]);
  // 渡るのは**バイト列**（供給元の形 — safetensors の view — は消費側に漏れない）。
  // 逐次消費側は転送後にこの参照を手放すので、実体はその shard のバイトで確定している
  assertEquals(fromWeightShard[0].payload, W_F32.data);
...
Deno.test("createShardValidator: 読了時に欠けている payload と scale を全件列挙する", () => {
  const validator = validatorFor(i8Graph());
...
Deno.test("createShardValidator: 2 本の i8 initializer が scale を共有するグラフを構築時に落とす", () => {
  ...
  const json = sharedScaleGraph(() => ({ dtype: "i8", scale: "m.s" }), { wa: [3, 4], wb: [3, 8] });

  const error = assertThrows(() => validatorFor(json), ContainerError, "共有されている");
  // 帰属が分かる診断 MUST — 名前が出ないと直す側はどちらを改名するか決められない
  // （名前は合流後の実体キー = 'm.wa' / 'm.wb'）
  assertEquals(error.message.includes("'m.s'"), true, error.message);
  assertEquals(error.message.includes("'m.wa'"), true, error.message);
  assertEquals(error.message.includes("'m.wb'"), true, error.message);
});
...
Deno.test("createShardValidator: piece 列を宣言順に受理して rowOffset / first / last を返す", () => {
  ...
  assertEquals(first.map((item) => item.name), ["enc.w", "enc.b"]);
  assertEquals(first[0].payload, W_PIECE_1.data);
  assertEquals(first[0].piece, { rowOffset: 0, rows: 2, first: true, last: false });
  // 分割されていないテンソルは従来どおり piece 欄を持たない
  assertEquals(first[1].piece, undefined);

  const second = validator.intake(weightShard([W_PIECE_2]));
  assertEquals(second.map((item) => item.name), ["enc.w"]);
  assertEquals(second[0].piece, { rowOffset: 2, rows: 2, first: false, last: true });
  validator.finish();
});

Deno.test("createShardValidator: piece 列の companion scale は piece 1 とだけ同居する", () => {
  const validator = validatorFor(i8PieceGraph());
...
Deno.test("createShardValidator: piece の index の飛び・逆行・count の食い違いを落とす", () => {
  const skipped = validatorFor(baseGraph());
  ...
  const rewound = validatorFor(baseGraph());
  ...
  const miscounted = validatorFor(baseGraph());
...
Deno.test("createShardValidator: piece の dtype と残り次元の不一致を落とす", () => {
  const wrongDtype = validatorFor(baseGraph());
  assertThrows(
    () =>
      wrongDtype.intake(
  ...
  const wrongTail = validatorFor(baseGraph());
  assertThrows(
    () =>
      wrongTail.intake(
        weightShard([
...
Deno.test("createShardValidator: piece の累積行数が宣言の先頭次元と合わない形を落とす", () => {
  ...
  const over = validatorFor(baseGraph());
  ...
  const under = validatorFor(baseGraph());
...
Deno.test("createShardValidator: 丸ごとと piece の混在を落とす", () => {
  const sameShard = validatorFor(baseGraph());
  assertThrows(
  ...
  const wholeFirst = validatorFor(baseGraph());
  ...
  const piecesFirst = validatorFor(baseGraph());
