...
/**
 * 決着と実測の置き場（`outputs/verify/<環境キー>/<日付>_depth-anything-golden/` — 消して安全）。
 *
 * 系列名を `<family>-golden` にするのは、models 側の e2e（`depth-anything`）が同じ根へ書くため
 * （同じ席に 2 モジュールが書くと互いの `cases` を上書きする）。
 */
const results = openResults("depth-anything-golden");
