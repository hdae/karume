 ...
 *
 * 実測（**全 25 モデル 60 出力**のうち f32 が 53 本。i32 / bool の 7 本は差 0 の厳密一致で、
 * この値の対象外 — ADR 0009）の最悪値は **maxAbs 3.58e-7**（conv2d_block output.0）/
 * **maxRel 1.77e-5**（batch_matmul output.0）。前者は f32 の eps（2^-23 ≈ 1.19e-7）の 3 ulp、
 * 後者はゼロ近傍の要素で相対誤差が伸びたもので、その要素の絶対誤差は 1.19e-7（= 1 ulp）に
 * すぎない。出所は近似ではなく演算の並べ替えで、① GPU 側の fma 融合 ② matmul / 行 reduce の
 * 縮約順序が torch と違う ③ 超越関数の実装差（WGSL 仕様は数 ulp を許す）— いずれも 1 出力
 * あたり数 ulp の桁。
 *
 * **判定は atol が主導する**（`|x−y| ≤ atol + rtol·|ref|`）: maxRel が rtol を上回る要素も
 * 絶対誤差が atol の 1/8 に収まっているので通る。atol 1e-6 は実測最悪 3.58e-7 の約 2.8 倍で、
 * 縮約長とチェーン長が伸びる余地を見た余裕はここに集約されている。実装バグ（op 取り違え・
 * 添字ずれ）の誤差は O(1) で、この閾値の 6 桁上に出る。
 *
