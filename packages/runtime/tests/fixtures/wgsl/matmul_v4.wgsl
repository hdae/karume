// karume matmul (a[m,k] · b[k,n], f32, レジスタ 128x128 タイル / 1 スレッド 8x8 / wg 16x16 + vec4)
struct Dims {
  m: u32,
  n: u32,
  k: u32,
...
fn main(
  ...
  if (ocq1 < n4) {
    if (orow0 < dims.m) {
      c[orow0 * n4 + ocq1] = acc0_1;
    }
    if (orow1 < dims.m) {
      c[orow1 * n4 + ocq1] = acc1_1;
    }
    if (orow2 < dims.m) {
      c[orow2 * n4 + ocq1] = acc2_1;
    }
    if (orow3 < dims.m) {
      c[orow3 * n4 + ocq1] = acc3_1;
    }
    if (orow4 < dims.m) {
      c[orow4 * n4 + ocq1] = acc4_1;
    }
    if (orow5 < dims.m) {
      c[orow5 * n4 + ocq1] = acc5_1;
    }
    if (orow6 < dims.m) {
      c[orow6 * n4 + ocq1] = acc6_1;
    }
    if (orow7 < dims.m) {
      c[orow7 * n4 + ocq1] = acc7_1;
    }
  }
}
