// karume linear (x[m,k] · wᵀ[k,n] + bias[n], 活性 per-token i8 × 重み i4 群 32 の整数内積・エミュ, タイル 128x64 / 1 スレッド 8x8 / wg 8x16 / K 16 + vec4)
struct Dims {
  m: u32,
  n: u32,
  k: u32,
...
fn main(
  ...
  if (ocq0 < n4) {
    ...
    if (orow1 < dims.m) {
      out[orow1 * n4 + ocq0] = fma(accf1_0, vec4<f32>(xscale[orow1]), biasv);
    }
    if (orow2 < dims.m) {
      out[orow2 * n4 + ocq0] = fma(accf2_0, vec4<f32>(xscale[orow2]), biasv);
    }
    if (orow3 < dims.m) {
      out[orow3 * n4 + ocq0] = fma(accf3_0, vec4<f32>(xscale[orow3]), biasv);
    }
    if (orow4 < dims.m) {
      out[orow4 * n4 + ocq0] = fma(accf4_0, vec4<f32>(xscale[orow4]), biasv);
    }
    if (orow5 < dims.m) {
      out[orow5 * n4 + ocq0] = fma(accf5_0, vec4<f32>(xscale[orow5]), biasv);
    }
    if (orow6 < dims.m) {
      out[orow6 * n4 + ocq0] = fma(accf6_0, vec4<f32>(xscale[orow6]), biasv);
    }
    if (orow7 < dims.m) {
      out[orow7 * n4 + ocq0] = fma(accf7_0, vec4<f32>(xscale[orow7]), biasv);
    }
  }
  if (ocq1 < n4) {
    ...
    // MUST: xs は最後の fma（単一丸め）へ回す — wscale は group 境界で畳み済み
    if (orow0 < dims.m) {
      out[orow0 * n4 + ocq1] = fma(accf0_1, vec4<f32>(xscale[orow0]), biasv);
    }
    if (orow1 < dims.m) {
      out[orow1 * n4 + ocq1] = fma(accf1_1, vec4<f32>(xscale[orow1]), biasv);
    }
    if (orow2 < dims.m) {
      out[orow2 * n4 + ocq1] = fma(accf2_1, vec4<f32>(xscale[orow2]), biasv);
    }
    if (orow3 < dims.m) {
      out[orow3 * n4 + ocq1] = fma(accf3_1, vec4<f32>(xscale[orow3]), biasv);
    }
    if (orow4 < dims.m) {
      out[orow4 * n4 + ocq1] = fma(accf4_1, vec4<f32>(xscale[orow4]), biasv);
    }
    if (orow5 < dims.m) {
      out[orow5 * n4 + ocq1] = fma(accf5_1, vec4<f32>(xscale[orow5]), biasv);
    }
    if (orow6 < dims.m) {
      out[orow6 * n4 + ocq1] = fma(accf6_1, vec4<f32>(xscale[orow6]), biasv);
    }
    if (orow7 < dims.m) {
      out[orow7 * n4 + ocq1] = fma(accf7_1, vec4<f32>(xscale[orow7]), biasv);
    }
  }
}
