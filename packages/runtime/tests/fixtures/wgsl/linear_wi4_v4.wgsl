// karume linear (x[m,k] · wᵀ[k,n] + bias[n], f32, 重み i4 格納, レジスタ 128x128 タイル / 1 スレッド 8x8 / wg 16x16 + vec4)
struct Dims {
  m: u32,
  n: u32,
  k: u32,
...
fn main(
  ...
  for (var t = 0u; t < tiles; t = t + 1u) {
    ...
    if (wcol0 < dims.n && wk0 < dims.k) {
      ...
      wv0 = dequant4(wrow_base0 + wk0, wgs0);
    }
    switch wsl0 {
      case 0u: {
        sb[sb_base0].x = wv0.x;
    ...
    if (wcol1 < dims.n && wk0 < dims.k) {
      ...
      wv1 = dequant4(wrow_base1 + wk0, wgs1);
    }
    switch wsl1 {
      case 0u: {
        sb[sb_base1].x = wv1.x;
