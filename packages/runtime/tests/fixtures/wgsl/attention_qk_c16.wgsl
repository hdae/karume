// karume attention_qk (S[b,m,n] = (q·scale)[b,m,d] · (k·scale)[b,n,d]ᵀ, f32, f16 タイル計算・S も f16, レジスタ 64x64 タイル)
enable f16;
struct Dims {
  m: u32,
  n: u32,
...
fn main(
  ...
  for (var i = 0u; i < 4u; i = i + 1u) {
    ...
    if (orow < dims.m) {
      ...
      if (ocol < dims.n) {
        s[obase + ocol] = f16(acc[i].x);
      }
      if (ocol + 1u < dims.n) {
        s[obase + ocol + 1u] = f16(acc[i].y);
      }
      if (ocol + 2u < dims.n) {
        s[obase + ocol + 2u] = f16(acc[i].z);
      }
      if (ocol + 3u < dims.n) {
        s[obase + ocol + 3u] = f16(acc[i].w);
      }
    }
  }
}
