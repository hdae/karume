// karume linear gemv K parallel (i8, 32 lanes/output, packed int8 活性)
struct Dims {
  m: u32,
  n: u32,
  k: u32,
...
fn main(@builtin(local_invocation_index) lid: u32, @builtin(workgroup_id) wg: vec3<u32>) {
  ...
  let wscale_v = wscale[col];
    ...
    let xat_1 = bitcast<vec4<f32>>(bitcast<vec4<u32>>(vec4<f32>(unpack4xI8(xpt_0.y)) * dims.x_scale) ^ vec4<u32>(dims.rounding_mask));
    ...
    let xat_2 = bitcast<vec4<f32>>(bitcast<vec4<u32>>(vec4<f32>(unpack4xI8(xpt_0.z)) * dims.x_scale) ^ vec4<u32>(dims.rounding_mask));
    ...
    let xat_3 = bitcast<vec4<f32>>(bitcast<vec4<u32>>(vec4<f32>(unpack4xI8(xpt_0.w)) * dims.x_scale) ^ vec4<u32>(dims.rounding_mask));
