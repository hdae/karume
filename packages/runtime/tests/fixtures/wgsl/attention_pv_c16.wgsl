// karume attention_pv (O[b,m,d] = P[b,m,n] · v[b,n,d], P = exp(S − m)·inv は非実体化, f32, f16 タイル計算・S は f16, レジスタ 64x64 タイル)
enable f16;
struct Dims {
  m: u32,
  n: u32,
...
fn main(
  ...
  for (var t = 0u; t < tiles; t = t + 1u) {
    ...
    if (arow < dims.m) {
      ...
      if (ak0 + 1u < dims.k) {
        av.y = exp(f32(s[arow_base + ak0 + 1u]) - row_max) * row_inv;
      }
      if (ak0 + 2u < dims.k) {
        av.z = exp(f32(s[arow_base + ak0 + 2u]) - row_max) * row_inv;
      }
      if (ak0 + 3u < dims.k) {
        av.w = exp(f32(s[arow_base + ak0 + 3u]) - row_max) * row_inv;
      }
    }
    sa[sa_base] = f16(av.x);
    ...
    if (brow < dims.k) {
      ...
      if (bcol < dims.n) {
        bv4.x = v[brow_base + bcol];
      }
      if (bcol + 1u < dims.n) {
        bv4.y = v[brow_base + bcol + 1u];
      }
      if (bcol + 2u < dims.n) {
        bv4.z = v[brow_base + bcol + 2u];
      }
      if (bcol + 3u < dims.n) {
        bv4.w = v[brow_base + bcol + 3u];
      }
    }
    sb[bk * 16u + bcq] = vec4<f16>(bv4);
    workgroupBarrier();
  ...
  for (var i = 0u; i < 4u; i = i + 1u) {
    ...
    if (orow < dims.m) {
      ...
      if (ocol < dims.n) {
        o[obase + ocol] = acc[i].x;
      }
      if (ocol + 1u < dims.n) {
        o[obase + ocol + 1u] = acc[i].y;
      }
      if (ocol + 2u < dims.n) {
        o[obase + ocol + 2u] = acc[i].z;
      }
      if (ocol + 3u < dims.n) {
        o[obase + ocol + 3u] = acc[i].w;
      }
    }
  }
}
