// karume attention_qk (S[b,m,n] = q[b,m,d] · k[b,n,d]ᵀ, q/k とも per-token i8 の整数内積・エミュ, 半スケールは dequant 側, S は f16 格納（pack2x16float）, レジスタ 64x64 タイル + vec4)
struct Dims {
  m: u32,
  n: u32,
  k: u32,
...
fn main(
  ...
  if (ocq < n4) {
    ...
    if (orow0 < dims.m) {
      score_store(sbase + orow0 * n4 + ocq, vec4<f32>(acc0) * ((qscale[qsbase + orow0] * dims.scale) * ks));
    }
    let orow1 = orow0 + 1u;
    if (orow1 < dims.m) {
      score_store(sbase + orow1 * n4 + ocq, vec4<f32>(acc1) * ((qscale[qsbase + orow1] * dims.scale) * ks));
    }
    let orow2 = orow0 + 2u;
    if (orow2 < dims.m) {
      score_store(sbase + orow2 * n4 + ocq, vec4<f32>(acc2) * ((qscale[qsbase + orow2] * dims.scale) * ks));
    }
    let orow3 = orow0 + 3u;
    if (orow3 < dims.m) {
      score_store(sbase + orow3 * n4 + ocq, vec4<f32>(acc3) * ((qscale[qsbase + orow3] * dims.scale) * ks));
    }
  }
}
