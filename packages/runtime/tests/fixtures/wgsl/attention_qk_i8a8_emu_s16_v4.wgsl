// karume attention_qk (S[b,m,n] = q[b,m,d] · k[b,n,d]ᵀ, q/k とも per-token i8 の整数内積・エミュ, 半スケールは dequant 側, S は f16 格納（pack2x16float）, タイル 128x64 / 1 スレッド 8x8 / wg 8x16 / K 16 + vec4)
struct Dims {
  m: u32,
  n: u32,
  k: u32,
...
fn main(
  ...
  if (ocq0 < n4) {
    ...
    if (orow0 < dims.m) {
      score_store(sbase + orow0 * n4 + ocq0, vec4<f32>(acc0_0) * ((qscale[qsbase + orow0] * dims.scale) * ks));
    }
    if (orow1 < dims.m) {
      score_store(sbase + orow1 * n4 + ocq0, vec4<f32>(acc1_0) * ((qscale[qsbase + orow1] * dims.scale) * ks));
    }
    if (orow2 < dims.m) {
      score_store(sbase + orow2 * n4 + ocq0, vec4<f32>(acc2_0) * ((qscale[qsbase + orow2] * dims.scale) * ks));
    }
    if (orow3 < dims.m) {
      score_store(sbase + orow3 * n4 + ocq0, vec4<f32>(acc3_0) * ((qscale[qsbase + orow3] * dims.scale) * ks));
    }
    if (orow4 < dims.m) {
      score_store(sbase + orow4 * n4 + ocq0, vec4<f32>(acc4_0) * ((qscale[qsbase + orow4] * dims.scale) * ks));
    }
    if (orow5 < dims.m) {
      score_store(sbase + orow5 * n4 + ocq0, vec4<f32>(acc5_0) * ((qscale[qsbase + orow5] * dims.scale) * ks));
    }
    if (orow6 < dims.m) {
      score_store(sbase + orow6 * n4 + ocq0, vec4<f32>(acc6_0) * ((qscale[qsbase + orow6] * dims.scale) * ks));
    }
    if (orow7 < dims.m) {
      score_store(sbase + orow7 * n4 + ocq0, vec4<f32>(acc7_0) * ((qscale[qsbase + orow7] * dims.scale) * ks));
    }
  }
  if (ocq1 < n4) {
    ...
    // MUST: qs'·ks' を先に 1 つの f32 へ畳んでから f32(acc) に掛ける（bias が無いので fma は無い）
    if (orow0 < dims.m) {
      score_store(sbase + orow0 * n4 + ocq1, vec4<f32>(acc0_1) * ((qscale[qsbase + orow0] * dims.scale) * ks));
    }
    if (orow1 < dims.m) {
      score_store(sbase + orow1 * n4 + ocq1, vec4<f32>(acc1_1) * ((qscale[qsbase + orow1] * dims.scale) * ks));
    }
    if (orow2 < dims.m) {
      score_store(sbase + orow2 * n4 + ocq1, vec4<f32>(acc2_1) * ((qscale[qsbase + orow2] * dims.scale) * ks));
    }
    if (orow3 < dims.m) {
      score_store(sbase + orow3 * n4 + ocq1, vec4<f32>(acc3_1) * ((qscale[qsbase + orow3] * dims.scale) * ks));
    }
    if (orow4 < dims.m) {
      score_store(sbase + orow4 * n4 + ocq1, vec4<f32>(acc4_1) * ((qscale[qsbase + orow4] * dims.scale) * ks));
    }
    if (orow5 < dims.m) {
      score_store(sbase + orow5 * n4 + ocq1, vec4<f32>(acc5_1) * ((qscale[qsbase + orow5] * dims.scale) * ks));
    }
    if (orow6 < dims.m) {
      score_store(sbase + orow6 * n4 + ocq1, vec4<f32>(acc6_1) * ((qscale[qsbase + orow6] * dims.scale) * ks));
    }
    if (orow7 < dims.m) {
      score_store(sbase + orow7 * n4 + ocq1, vec4<f32>(acc7_1) * ((qscale[qsbase + orow7] * dims.scale) * ks));
    }
  }
}
