// karume quantize_rows (行ごとの per-token symmetric i8: s = max(amax/127, tiny), q = clamp(round(x/s), ±127)) — 2 行 × 128 レーン / workgroup
struct Params {
  rows: u32,
  dim: u32,
}
...
fn main(
  ...
  let quads = dim / 4u;
  // レーン組: 幅 128 × 2 組。組の中の位置 lane と組の番号 slot
  let lane = lid % 128u;
  let slot = lid / 128u;
  var rowBase = wid.x * 2u;
