...
fn main(
  ...
  for (var i = 0u; i < 4u; i = i + 1u) {
    ...
    if (orow < dims.m) {
      ...
      if (ocol + 1u < dims.n) {
        o[orow_base + ocol + 1u] = f32(acc[i].y) * (prow * vscale[vsbase + ocol + 1u]);
      }
      if (ocol + 2u < dims.n) {
        o[orow_base + ocol + 2u] = f32(acc[i].z) * (prow * vscale[vsbase + ocol + 2u]);
      }
      if (ocol + 3u < dims.n) {
        o[orow_base + ocol + 3u] = f32(acc[i].w) * (prow * vscale[vsbase + ocol + 3u]);
      }
    }
  }
}
