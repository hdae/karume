// karume attention_pv (O[b,m,d] = P[b,m,n] · v[b,n,d], P̃ = round(127·exp(S − m)) は非実体化・エミュ, v は per-column i8（Vᵀ 連続）, レジスタ 64x64 タイル)
struct Dims {
  m: u32,
  n: u32,
  k: u32,
...
fn main(
  ...
  if (orow0 < dims.m) {
    ...
    if (ocol + 1u < dims.n) {
      o[orow_base + ocol + 1u] = f32(acc0.y) * (prow * vscale[vsbase + ocol + 1u]);
    }
    if (ocol + 2u < dims.n) {
      o[orow_base + ocol + 2u] = f32(acc0.z) * (prow * vscale[vsbase + ocol + 2u]);
    }
    if (ocol + 3u < dims.n) {
      o[orow_base + ocol + 3u] = f32(acc0.w) * (prow * vscale[vsbase + ocol + 3u]);
    }
  }
  let orow1 = orow0 + 1u;
  if (orow1 < dims.m) {
    ...
    if (ocol < dims.n) {
      o[orow_base + ocol] = f32(acc1.x) * (prow * vscale[vsbase + ocol]);
    }
    if (ocol + 1u < dims.n) {
      o[orow_base + ocol + 1u] = f32(acc1.y) * (prow * vscale[vsbase + ocol + 1u]);
    }
    if (ocol + 2u < dims.n) {
      o[orow_base + ocol + 2u] = f32(acc1.z) * (prow * vscale[vsbase + ocol + 2u]);
    }
    if (ocol + 3u < dims.n) {
      o[orow_base + ocol + 3u] = f32(acc1.w) * (prow * vscale[vsbase + ocol + 3u]);
    }
  }
  let orow2 = orow0 + 2u;
  if (orow2 < dims.m) {
    ...
    if (ocol < dims.n) {
      o[orow_base + ocol] = f32(acc2.x) * (prow * vscale[vsbase + ocol]);
    }
    if (ocol + 1u < dims.n) {
      o[orow_base + ocol + 1u] = f32(acc2.y) * (prow * vscale[vsbase + ocol + 1u]);
    }
    if (ocol + 2u < dims.n) {
      o[orow_base + ocol + 2u] = f32(acc2.z) * (prow * vscale[vsbase + ocol + 2u]);
    }
    if (ocol + 3u < dims.n) {
      o[orow_base + ocol + 3u] = f32(acc2.w) * (prow * vscale[vsbase + ocol + 3u]);
    }
  }
  let orow3 = orow0 + 3u;
  if (orow3 < dims.m) {
    ...
    if (ocol < dims.n) {
      o[orow_base + ocol] = f32(acc3.x) * (prow * vscale[vsbase + ocol]);
    }
    if (ocol + 1u < dims.n) {
      o[orow_base + ocol + 1u] = f32(acc3.y) * (prow * vscale[vsbase + ocol + 1u]);
    }
    if (ocol + 2u < dims.n) {
      o[orow_base + ocol + 2u] = f32(acc3.z) * (prow * vscale[vsbase + ocol + 2u]);
    }
    if (ocol + 3u < dims.n) {
      o[orow_base + ocol + 3u] = f32(acc3.w) * (prow * vscale[vsbase + ocol + 3u]);
    }
  }
}
