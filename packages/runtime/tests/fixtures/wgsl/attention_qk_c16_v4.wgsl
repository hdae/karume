// karume attention_qk (S[b,m,n] = (q·scale)[b,m,d] · (k·scale)[b,n,d]ᵀ, f32, f16 タイル計算・S も f16, レジスタ 128x128 タイル / 1 スレッド 8x8 / wg 16x16 + vec4)
enable f16;
struct Dims {
  m: u32,
  n: u32,
...
@group(0) @binding(3) var<storage, read_write> s: array<vec4<f16>>;
...
fn main(
  ...
  if (ocq0 < n4) {
    if (orow0 < dims.m) {
      s[cbase + orow0 * n4 + ocq0] = vec4<f16>(acc0_0);
    }
    if (orow1 < dims.m) {
      s[cbase + orow1 * n4 + ocq0] = vec4<f16>(acc1_0);
    }
    if (orow2 < dims.m) {
      s[cbase + orow2 * n4 + ocq0] = vec4<f16>(acc2_0);
    }
    if (orow3 < dims.m) {
      s[cbase + orow3 * n4 + ocq0] = vec4<f16>(acc3_0);
    }
    if (orow4 < dims.m) {
      s[cbase + orow4 * n4 + ocq0] = vec4<f16>(acc4_0);
    }
    if (orow5 < dims.m) {
      s[cbase + orow5 * n4 + ocq0] = vec4<f16>(acc5_0);
    }
    if (orow6 < dims.m) {
      s[cbase + orow6 * n4 + ocq0] = vec4<f16>(acc6_0);
    }
    if (orow7 < dims.m) {
      s[cbase + orow7 * n4 + ocq0] = vec4<f16>(acc7_0);
    }
  }
  if (ocq1 < n4) {
    if (orow0 < dims.m) {
      s[cbase + orow0 * n4 + ocq1] = vec4<f16>(acc0_1);
    }
    if (orow1 < dims.m) {
      s[cbase + orow1 * n4 + ocq1] = vec4<f16>(acc1_1);
    }
    if (orow2 < dims.m) {
      s[cbase + orow2 * n4 + ocq1] = vec4<f16>(acc2_1);
    }
    if (orow3 < dims.m) {
      s[cbase + orow3 * n4 + ocq1] = vec4<f16>(acc3_1);
    }
    if (orow4 < dims.m) {
      s[cbase + orow4 * n4 + ocq1] = vec4<f16>(acc4_1);
    }
    if (orow5 < dims.m) {
      s[cbase + orow5 * n4 + ocq1] = vec4<f16>(acc5_1);
    }
    if (orow6 < dims.m) {
      s[cbase + orow6 * n4 + ocq1] = vec4<f16>(acc6_1);
    }
    if (orow7 < dims.m) {
      s[cbase + orow7 * n4 + ocq1] = vec4<f16>(acc7_1);
    }
  }
}
