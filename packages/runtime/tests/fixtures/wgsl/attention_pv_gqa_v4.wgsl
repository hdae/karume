// karume attention_pv (O[b,m,d] = P[b,m,n] · v[b,n,d], P = exp(S − m)·inv は非実体化, f32, GQA（v は kv-head へ整数除算で写す）, レジスタ 128x128 タイル / 1 スレッド 8x8 / wg 16x16 + vec4)
struct Dims {
  m: u32,
  n: u32,
  k: u32,
...
fn main(
  ...
  let bbase = (wid.z / dims.kv_repeat) * dims.k * n4;
