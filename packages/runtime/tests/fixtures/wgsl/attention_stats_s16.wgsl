// karume attention_stats (行ごとの m = amax(S) と inv = 1/Σexp(S - m), f32, S は f16 格納（pack2x16float）)
struct Params {
  rows: u32,
  dim: u32,
  neg_inf: u32,
...
// f16 格納の展開: 要素 i = unpack2x16float(s[i / 2])[i % 2]（平坦添字の偶奇で対を選ぶ）
fn score_at(i: u32) -> f32 {
  let pair = unpack2x16float(s[i >> 1u]);
...
fn main(
  ...
  while (row < params.rows) {
    ...
    var j = lid;
    while (j < dim) {
      acc = acc + exp(score_at(base + j) - amax);
      j = j + 256u;
    }
    scratch[lid] = acc;
    workgroupBarrier();
