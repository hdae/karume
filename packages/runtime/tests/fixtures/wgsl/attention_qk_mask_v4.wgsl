// karume attention_qk (S[b,m,n] = (q·scale)[b,m,d] · (k·scale)[b,n,d]ᵀ + mask[m,n], f32, レジスタ 128x128 タイル / 1 スレッド 8x8 / wg 16x16 + vec4)
struct Dims {
  m: u32,
  n: u32,
  k: u32,
...
fn main(
  ...
  if (ocq0 < n4) {
    ...
    if (orow1 < dims.m) {
      let sv = acc1_0 + mask[orow1 * n4 + ocq0];
      s[cbase + orow1 * n4 + ocq0] = sv;
    }
    if (orow2 < dims.m) {
      let sv = acc2_0 + mask[orow2 * n4 + ocq0];
      s[cbase + orow2 * n4 + ocq0] = sv;
    }
    if (orow3 < dims.m) {
      let sv = acc3_0 + mask[orow3 * n4 + ocq0];
      s[cbase + orow3 * n4 + ocq0] = sv;
    }
    if (orow4 < dims.m) {
      let sv = acc4_0 + mask[orow4 * n4 + ocq0];
      s[cbase + orow4 * n4 + ocq0] = sv;
    }
    if (orow5 < dims.m) {
      let sv = acc5_0 + mask[orow5 * n4 + ocq0];
      s[cbase + orow5 * n4 + ocq0] = sv;
    }
    if (orow6 < dims.m) {
      let sv = acc6_0 + mask[orow6 * n4 + ocq0];
      s[cbase + orow6 * n4 + ocq0] = sv;
    }
    if (orow7 < dims.m) {
      let sv = acc7_0 + mask[orow7 * n4 + ocq0];
      s[cbase + orow7 * n4 + ocq0] = sv;
    }
  }
  if (ocq1 < n4) {
    if (orow0 < dims.m) {
      let sv = acc0_1 + mask[orow0 * n4 + ocq1];
      s[cbase + orow0 * n4 + ocq1] = sv;
    }
    if (orow1 < dims.m) {
      let sv = acc1_1 + mask[orow1 * n4 + ocq1];
      s[cbase + orow1 * n4 + ocq1] = sv;
    }
    if (orow2 < dims.m) {
      let sv = acc2_1 + mask[orow2 * n4 + ocq1];
      s[cbase + orow2 * n4 + ocq1] = sv;
    }
    if (orow3 < dims.m) {
      let sv = acc3_1 + mask[orow3 * n4 + ocq1];
      s[cbase + orow3 * n4 + ocq1] = sv;
    }
    if (orow4 < dims.m) {
      let sv = acc4_1 + mask[orow4 * n4 + ocq1];
      s[cbase + orow4 * n4 + ocq1] = sv;
    }
    if (orow5 < dims.m) {
      let sv = acc5_1 + mask[orow5 * n4 + ocq1];
      s[cbase + orow5 * n4 + ocq1] = sv;
    }
    if (orow6 < dims.m) {
      let sv = acc6_1 + mask[orow6 * n4 + ocq1];
      s[cbase + orow6 * n4 + ocq1] = sv;
    }
    if (orow7 < dims.m) {
      let sv = acc7_1 + mask[orow7 * n4 + ocq1];
