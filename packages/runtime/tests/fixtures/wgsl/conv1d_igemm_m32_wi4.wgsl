// karume conv1d (x[B,Cin,L] * W[Cout,Cin,K] + b[Cout], f32, 重み i4 格納, implicit GEMM レジスタ 32x128 タイル / 1 スレッド 8x8 / wg 16x4)
struct Dims {
  m: u32,
  n: u32,
  k: u32,
...
fn main(
  ...
  for (var t = 0u; t < tiles; t = t + 1u) {
    ...
    if (arow0 < dims.m) {
      ...
      let ags0 = wscale[arow0 * (dims.k >> 5u) + (ak0 >> 5u)];
      if (ak0 < dims.k) {
        av0.x = dequant(abase, ags0);
      }
      if (ak0 + 1u < dims.k) {
        av0.y = dequant(abase + 1u, ags0);
      }
      if (ak0 + 2u < dims.k) {
        av0.z = dequant(abase + 2u, ags0);
      }
      if (ak0 + 3u < dims.k) {
        av0.w = dequant(abase + 3u, ags0);
      }
    }
    sa[sa_base0] = av0.x;
    ...
    if (arow1 < dims.m) {
      ...
      let ags1 = wscale[arow1 * (dims.k >> 5u) + (ak0 >> 5u)];
      if (ak0 < dims.k) {
        av1.x = dequant(abase, ags1);
      }
      if (ak0 + 1u < dims.k) {
        av1.y = dequant(abase + 1u, ags1);
      }
      if (ak0 + 2u < dims.k) {
        av1.z = dequant(abase + 2u, ags1);
      }
      if (ak0 + 3u < dims.k) {
        av1.w = dequant(abase + 3u, ags1);
      }
    }
    sa[sa_base1] = av1.x;
