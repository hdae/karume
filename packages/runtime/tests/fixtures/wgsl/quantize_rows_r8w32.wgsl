// karume quantize_rows (行ごとの per-token symmetric i8: s = max(amax/127, tiny), q = clamp(round(x/s), ±127)) — 8 行 × 32 レーン / workgroup
struct Params {
  rows: u32,
  dim: u32,
}
...
fn main(
  ...
  let quads = dim / 4u;
  // レーン組: 幅 32 × 8 組。組の中の位置 lane と組の番号 slot
  let lane = lid % 32u;
  let slot = lid / 32u;
  var rowBase = wid.x * 8u;
