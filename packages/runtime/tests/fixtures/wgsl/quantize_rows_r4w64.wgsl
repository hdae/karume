// karume quantize_rows (行ごとの per-token symmetric i8: s = max(amax/127, tiny), q = clamp(round(x/s), ±127)) — 4 行 × 64 レーン / workgroup
struct Params {
  rows: u32,
  dim: u32,
}
...
fn main(
  ...
  let quads = dim / 4u;
  // レーン組: 幅 64 × 4 組。組の中の位置 lane と組の番号 slot
  let lane = lid % 64u;
  let slot = lid / 64u;
  var rowBase = wid.x * 4u;
