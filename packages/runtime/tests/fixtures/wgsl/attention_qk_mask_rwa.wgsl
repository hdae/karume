// karume attention_qk (S[b,m,n] = (q·scale)[b,m,d] · (k·scale)[b,n,d]ᵀ + mask[m,n], f32, 行窓 a, レジスタ 128x128 タイル / 1 スレッド 8x8 / wg 16x16)
struct Dims {
  m: u32,
  n: u32,
  k: u32,
...
fn main(
  ...
  if (orow0 < dims.m) {
    ...
    let mbase = (orow0 + dims.row_offset) * dims.n;
    if (ocol < dims.n) {
      let sv = acc0_0.x + mask[mbase + ocol];
      s[obase + ocol] = sv;
    }
    if (ocol + 1u < dims.n) {
      let sv = acc0_0.y + mask[mbase + ocol + 1u];
      s[obase + ocol + 1u] = sv;
    }
    if (ocol + 2u < dims.n) {
      let sv = acc0_0.z + mask[mbase + ocol + 2u];
      s[obase + ocol + 2u] = sv;
    }
    if (ocol + 3u < dims.n) {
      let sv = acc0_0.w + mask[mbase + ocol + 3u];
      s[obase + ocol + 3u] = sv;
    }
    if (ocol + 4u < dims.n) {
      let sv = acc0_1.x + mask[mbase + ocol + 4u];
      s[obase + ocol + 4u] = sv;
    }
    if (ocol + 5u < dims.n) {
      let sv = acc0_1.y + mask[mbase + ocol + 5u];
      s[obase + ocol + 5u] = sv;
    }
    if (ocol + 6u < dims.n) {
      let sv = acc0_1.z + mask[mbase + ocol + 6u];
      s[obase + ocol + 6u] = sv;
    }
    if (ocol + 7u < dims.n) {
      let sv = acc0_1.w + mask[mbase + ocol + 7u];
      s[obase + ocol + 7u] = sv;
    }
  }
  if (orow1 < dims.m) {
    let obase = cbase + orow1 * dims.n;
    let mbase = (orow1 + dims.row_offset) * dims.n;
    if (ocol < dims.n) {
      let sv = acc1_0.x + mask[mbase + ocol];
      s[obase + ocol] = sv;
    }
    if (ocol + 1u < dims.n) {
      let sv = acc1_0.y + mask[mbase + ocol + 1u];
      s[obase + ocol + 1u] = sv;
    }
    if (ocol + 2u < dims.n) {
      let sv = acc1_0.z + mask[mbase + ocol + 2u];
      s[obase + ocol + 2u] = sv;
    }
    if (ocol + 3u < dims.n) {
      let sv = acc1_0.w + mask[mbase + ocol + 3u];
      s[obase + ocol + 3u] = sv;
    }
    if (ocol + 4u < dims.n) {
      let sv = acc1_1.x + mask[mbase + ocol + 4u];
      s[obase + ocol + 4u] = sv;
    }
    if (ocol + 5u < dims.n) {
      let sv = acc1_1.y + mask[mbase + ocol + 5u];
      s[obase + ocol + 5u] = sv;
    }
    if (ocol + 6u < dims.n) {
      let sv = acc1_1.z + mask[mbase + ocol + 6u];
      s[obase + ocol + 6u] = sv;
    }
    if (ocol + 7u < dims.n) {
      let sv = acc1_1.w + mask[mbase + ocol + 7u];
      s[obase + ocol + 7u] = sv;
    }
  }
  if (orow2 < dims.m) {
    let obase = cbase + orow2 * dims.n;
    let mbase = (orow2 + dims.row_offset) * dims.n;
    if (ocol < dims.n) {
      let sv = acc2_0.x + mask[mbase + ocol];
      s[obase + ocol] = sv;
    }
    if (ocol + 1u < dims.n) {
      let sv = acc2_0.y + mask[mbase + ocol + 1u];
      s[obase + ocol + 1u] = sv;
    }
    if (ocol + 2u < dims.n) {
      let sv = acc2_0.z + mask[mbase + ocol + 2u];
      s[obase + ocol + 2u] = sv;
    }
    if (ocol + 3u < dims.n) {
      let sv = acc2_0.w + mask[mbase + ocol + 3u];
      s[obase + ocol + 3u] = sv;
    }
    if (ocol + 4u < dims.n) {
      let sv = acc2_1.x + mask[mbase + ocol + 4u];
      s[obase + ocol + 4u] = sv;
    }
    if (ocol + 5u < dims.n) {
      let sv = acc2_1.y + mask[mbase + ocol + 5u];
      s[obase + ocol + 5u] = sv;
    }
    if (ocol + 6u < dims.n) {
      let sv = acc2_1.z + mask[mbase + ocol + 6u];
      s[obase + ocol + 6u] = sv;
    }
    if (ocol + 7u < dims.n) {
      let sv = acc2_1.w + mask[mbase + ocol + 7u];
      s[obase + ocol + 7u] = sv;
    }
  }
  if (orow3 < dims.m) {
    let obase = cbase + orow3 * dims.n;
    let mbase = (orow3 + dims.row_offset) * dims.n;
    if (ocol < dims.n) {
      let sv = acc3_0.x + mask[mbase + ocol];
      s[obase + ocol] = sv;
    }
    if (ocol + 1u < dims.n) {
      let sv = acc3_0.y + mask[mbase + ocol + 1u];
      s[obase + ocol + 1u] = sv;
    }
    if (ocol + 2u < dims.n) {
      let sv = acc3_0.z + mask[mbase + ocol + 2u];
      s[obase + ocol + 2u] = sv;
    }
    if (ocol + 3u < dims.n) {
      let sv = acc3_0.w + mask[mbase + ocol + 3u];
      s[obase + ocol + 3u] = sv;
    }
    if (ocol + 4u < dims.n) {
      let sv = acc3_1.x + mask[mbase + ocol + 4u];
      s[obase + ocol + 4u] = sv;
    }
    if (ocol + 5u < dims.n) {
      let sv = acc3_1.y + mask[mbase + ocol + 5u];
      s[obase + ocol + 5u] = sv;
    }
    if (ocol + 6u < dims.n) {
      let sv = acc3_1.z + mask[mbase + ocol + 6u];
      s[obase + ocol + 6u] = sv;
    }
    if (ocol + 7u < dims.n) {
      let sv = acc3_1.w + mask[mbase + ocol + 7u];
      s[obase + ocol + 7u] = sv;
    }
  }
  if (orow4 < dims.m) {
    let obase = cbase + orow4 * dims.n;
    let mbase = (orow4 + dims.row_offset) * dims.n;
    if (ocol < dims.n) {
      let sv = acc4_0.x + mask[mbase + ocol];
      s[obase + ocol] = sv;
    }
    if (ocol + 1u < dims.n) {
      let sv = acc4_0.y + mask[mbase + ocol + 1u];
      s[obase + ocol + 1u] = sv;
    }
    if (ocol + 2u < dims.n) {
      let sv = acc4_0.z + mask[mbase + ocol + 2u];
      s[obase + ocol + 2u] = sv;
    }
    if (ocol + 3u < dims.n) {
      let sv = acc4_0.w + mask[mbase + ocol + 3u];
      s[obase + ocol + 3u] = sv;
    }
    if (ocol + 4u < dims.n) {
      let sv = acc4_1.x + mask[mbase + ocol + 4u];
      s[obase + ocol + 4u] = sv;
    }
    if (ocol + 5u < dims.n) {
      let sv = acc4_1.y + mask[mbase + ocol + 5u];
      s[obase + ocol + 5u] = sv;
    }
    if (ocol + 6u < dims.n) {
      let sv = acc4_1.z + mask[mbase + ocol + 6u];
      s[obase + ocol + 6u] = sv;
    }
    if (ocol + 7u < dims.n) {
      let sv = acc4_1.w + mask[mbase + ocol + 7u];
      s[obase + ocol + 7u] = sv;
    }
  }
  if (orow5 < dims.m) {
    let obase = cbase + orow5 * dims.n;
    let mbase = (orow5 + dims.row_offset) * dims.n;
    if (ocol < dims.n) {
      let sv = acc5_0.x + mask[mbase + ocol];
      s[obase + ocol] = sv;
    }
    if (ocol + 1u < dims.n) {
      let sv = acc5_0.y + mask[mbase + ocol + 1u];
      s[obase + ocol + 1u] = sv;
    }
    if (ocol + 2u < dims.n) {
      let sv = acc5_0.z + mask[mbase + ocol + 2u];
      s[obase + ocol + 2u] = sv;
    }
    if (ocol + 3u < dims.n) {
      let sv = acc5_0.w + mask[mbase + ocol + 3u];
      s[obase + ocol + 3u] = sv;
    }
    if (ocol + 4u < dims.n) {
      let sv = acc5_1.x + mask[mbase + ocol + 4u];
      s[obase + ocol + 4u] = sv;
    }
    if (ocol + 5u < dims.n) {
      let sv = acc5_1.y + mask[mbase + ocol + 5u];
      s[obase + ocol + 5u] = sv;
    }
    if (ocol + 6u < dims.n) {
      let sv = acc5_1.z + mask[mbase + ocol + 6u];
      s[obase + ocol + 6u] = sv;
    }
    if (ocol + 7u < dims.n) {
      let sv = acc5_1.w + mask[mbase + ocol + 7u];
      s[obase + ocol + 7u] = sv;
    }
  }
  if (orow6 < dims.m) {
    let obase = cbase + orow6 * dims.n;
    let mbase = (orow6 + dims.row_offset) * dims.n;
    if (ocol < dims.n) {
      let sv = acc6_0.x + mask[mbase + ocol];
      s[obase + ocol] = sv;
    }
    if (ocol + 1u < dims.n) {
      let sv = acc6_0.y + mask[mbase + ocol + 1u];
      s[obase + ocol + 1u] = sv;
    }
    if (ocol + 2u < dims.n) {
      let sv = acc6_0.z + mask[mbase + ocol + 2u];
      s[obase + ocol + 2u] = sv;
    }
    if (ocol + 3u < dims.n) {
      let sv = acc6_0.w + mask[mbase + ocol + 3u];
      s[obase + ocol + 3u] = sv;
    }
    if (ocol + 4u < dims.n) {
      let sv = acc6_1.x + mask[mbase + ocol + 4u];
      s[obase + ocol + 4u] = sv;
    }
    if (ocol + 5u < dims.n) {
      let sv = acc6_1.y + mask[mbase + ocol + 5u];
      s[obase + ocol + 5u] = sv;
    }
    if (ocol + 6u < dims.n) {
      let sv = acc6_1.z + mask[mbase + ocol + 6u];
      s[obase + ocol + 6u] = sv;
    }
    if (ocol + 7u < dims.n) {
      let sv = acc6_1.w + mask[mbase + ocol + 7u];
      s[obase + ocol + 7u] = sv;
    }
  }
  if (orow7 < dims.m) {
    let obase = cbase + orow7 * dims.n;
    let mbase = (orow7 + dims.row_offset) * dims.n;
    if (ocol < dims.n) {
      let sv = acc7_0.x + mask[mbase + ocol];
      s[obase + ocol] = sv;
    }
    if (ocol + 1u < dims.n) {
      let sv = acc7_0.y + mask[mbase + ocol + 1u];
      s[obase + ocol + 1u] = sv;
    }
    if (ocol + 2u < dims.n) {
      let sv = acc7_0.z + mask[mbase + ocol + 2u];
      s[obase + ocol + 2u] = sv;
    }
    if (ocol + 3u < dims.n) {
      let sv = acc7_0.w + mask[mbase + ocol + 3u];
      s[obase + ocol + 3u] = sv;
    }
    if (ocol + 4u < dims.n) {
      let sv = acc7_1.x + mask[mbase + ocol + 4u];
      s[obase + ocol + 4u] = sv;
    }
    if (ocol + 5u < dims.n) {
      let sv = acc7_1.y + mask[mbase + ocol + 5u];
      s[obase + ocol + 5u] = sv;
    }
    if (ocol + 6u < dims.n) {
      let sv = acc7_1.z + mask[mbase + ocol + 6u];
      s[obase + ocol + 6u] = sv;
    }
    if (ocol + 7u < dims.n) {
      let sv = acc7_1.w + mask[mbase + ocol + 7u];
      s[obase + ocol + 7u] = sv;
    }
  }
}
