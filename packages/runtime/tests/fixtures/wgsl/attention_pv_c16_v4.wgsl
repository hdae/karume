// karume attention_pv (O[b,m,d] = P[b,m,n] · v[b,n,d], P = exp(S − m)·inv は非実体化, f32, f16 タイル計算・S は f16, レジスタ 64x64 タイル + vec4)
enable f16;
struct Dims {
  m: u32,
  n: u32,
...
fn main(
  ...
  for (var t = 0u; t < tiles; t = t + 1u) {
    ...
    if (arow < dims.m && ak0 < dims.k) {
      ...
      av = vec4<f32>(
        exp(f32(raw.x) - row_max) * row_inv,
        exp(f32(raw.y) - row_max) * row_inv,
        exp(f32(raw.z) - row_max) * row_inv,
        exp(f32(raw.w) - row_max) * row_inv,
      );
    }
    sa[sa_base] = f16(av.x);
