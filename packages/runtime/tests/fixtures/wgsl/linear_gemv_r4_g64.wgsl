// karume linear gemv rows (M≥2: out[m,n] = x[m,k] · wᵀ[n,k] + bias[n], f32, 重み i4 格納, 32 列 / wg, 語 4 本先読み, 4 行 / スレッド)
struct Dims {
  m: u32,
  n: u32,
  k: u32,
...
fn main(@builtin(global_invocation_id) gid: vec3<u32>) {
  ...
  for (; unit + 4u <= units; unit = unit + 4u) {
    ...
    let ws0 = wscale[scale_base + ((unit0 * 32u) >> 6u)];
    let xq0 = unit0 * 8u;
    let unit1 = unit + 1u;
    let pw1 = w[row_base + unit1];
    let ws1 = wscale[scale_base + ((unit1 * 32u) >> 6u)];
    let xq1 = unit1 * 8u;
    let unit2 = unit + 2u;
    let pw2 = w[row_base + unit2];
    let ws2 = wscale[scale_base + ((unit2 * 32u) >> 6u)];
    let xq2 = unit2 * 8u;
    let unit3 = unit + 3u;
    let pw3 = w[row_base + unit3];
    let ws3 = wscale[scale_base + ((unit3 * 32u) >> 6u)];
    let xq3 = unit3 * 8u;
    acc0 = linear_i4_word(pw0, xr0 + xq0, ws0, acc0);
    acc1 = linear_i4_word(pw0, xr1 + xq0, ws0, acc1);
    acc2 = linear_i4_word(pw0, xr2 + xq0, ws0, acc2);
    acc3 = linear_i4_word(pw0, xr3 + xq0, ws0, acc3);
    acc0 = linear_i4_word(pw1, xr0 + xq1, ws1, acc0);
    acc1 = linear_i4_word(pw1, xr1 + xq1, ws1, acc1);
    acc2 = linear_i4_word(pw1, xr2 + xq1, ws1, acc2);
    acc3 = linear_i4_word(pw1, xr3 + xq1, ws1, acc3);
    acc0 = linear_i4_word(pw2, xr0 + xq2, ws2, acc0);
    acc1 = linear_i4_word(pw2, xr1 + xq2, ws2, acc1);
    acc2 = linear_i4_word(pw2, xr2 + xq2, ws2, acc2);
    acc3 = linear_i4_word(pw2, xr3 + xq2, ws2, acc3);
    acc0 = linear_i4_word(pw3, xr0 + xq3, ws3, acc0);
    acc1 = linear_i4_word(pw3, xr1 + xq3, ws3, acc1);
    acc2 = linear_i4_word(pw3, xr2 + xq3, ws3, acc2);
    acc3 = linear_i4_word(pw3, xr3 + xq3, ws3, acc3);
  }
  // 端数の語（units % 4 本）— 上と同じ順序を 1 語ずつ辿る
  for (; unit < units; unit = unit + 1u) {
    let unitt = unit;
    let pwt = w[row_base + unitt];
    let wst = wscale[scale_base + ((unitt * 32u) >> 6u)];
    let xqt = unitt * 8u;
    acc0 = linear_i4_word(pwt, xr0 + xqt, wst, acc0);
    acc1 = linear_i4_word(pwt, xr1 + xqt, wst, acc1);
    acc2 = linear_i4_word(pwt, xr2 + xqt, wst, acc2);
    acc3 = linear_i4_word(pwt, xr3 + xqt, wst, acc3);
  }
  if (row0 + 0u < dims.m) {
