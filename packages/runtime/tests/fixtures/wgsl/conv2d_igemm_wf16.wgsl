// karume conv2d (x[B,Cin,H,W] * W[Cout,Cin,Kh,Kw] + b[Cout], f32, 重み f16 格納, implicit GEMM レジスタ 64x64 タイル)
struct Dims {
  m: u32,
  n: u32,
  k: u32,
...
fn main(
  ...
  for (var t = 0u; t < tiles; t = t + 1u) {
    ...
    if (arow < dims.m) {
      ...
      if (ak0 + 1u < dims.k) {
        av.y = dequant(abase + 1u);
      }
      if (ak0 + 2u < dims.k) {
        av.z = dequant(abase + 2u);
      }
      if (ak0 + 3u < dims.k) {
        av.w = dequant(abase + 3u);
      }
    }
    sa[sa_base] = av.x;
    sa[sa_base + 1u] = av.y;
