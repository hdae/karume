// karume attention_pv (O[b,m,d] = P[b,m,n] · v[b,n,d], P = exp(S − m)·inv は非実体化, f32, レジスタ 64x64 タイル)
struct Dims {
  m: u32,
  n: u32,
  k: u32,
...
fn main(
  ...
  for (var t = 0u; t < tiles; t = t + 1u) {
    ...
    if (arow < dims.m) {
      if (ak0 < dims.k) {
        av.x = exp(s[arow_base + ak0] - row_max) * row_inv;
      }
      if (ak0 + 1u < dims.k) {
        av.y = exp(s[arow_base + ak0 + 1u] - row_max) * row_inv;
      }
      if (ak0 + 2u < dims.k) {
        av.z = exp(s[arow_base + ak0 + 2u] - row_max) * row_inv;
      }
      if (ak0 + 3u < dims.k) {
        av.w = exp(s[arow_base + ak0 + 3u] - row_max) * row_inv;
      }
    }
    sa[sa_base] = av.x;
    sa[sa_base + 1u] = av.y;
