// karume attention_qk (S[b,m,n] = (q·scale)[b,m,d] · (k·scale)[b,n,d]ᵀ, f32, S は f16 格納（pack2x16float）, レジスタ 128x128 タイル / 1 スレッド 8x8 / wg 16x16 + vec4)
struct Dims {
  m: u32,
  n: u32,
  k: u32,
...
fn main(
  ...
  if (ocq0 < n4) {
    ...
    if (orow1 < dims.m) {
      score_store(cbase + orow1 * n4 + ocq0, acc1_0);
    }
    if (orow2 < dims.m) {
      score_store(cbase + orow2 * n4 + ocq0, acc2_0);
    }
    if (orow3 < dims.m) {
      score_store(cbase + orow3 * n4 + ocq0, acc3_0);
    }
    if (orow4 < dims.m) {
      score_store(cbase + orow4 * n4 + ocq0, acc4_0);
    }
    if (orow5 < dims.m) {
      score_store(cbase + orow5 * n4 + ocq0, acc5_0);
    }
    if (orow6 < dims.m) {
      score_store(cbase + orow6 * n4 + ocq0, acc6_0);
    }
    if (orow7 < dims.m) {
      score_store(cbase + orow7 * n4 + ocq0, acc7_0);
    }
  }
  if (ocq1 < n4) {
    if (orow0 < dims.m) {
      score_store(cbase + orow0 * n4 + ocq1, acc0_1);
    }
    if (orow1 < dims.m) {
      score_store(cbase + orow1 * n4 + ocq1, acc1_1);
    }
    if (orow2 < dims.m) {
      score_store(cbase + orow2 * n4 + ocq1, acc2_1);
    }
    if (orow3 < dims.m) {
      score_store(cbase + orow3 * n4 + ocq1, acc3_1);
    }
    if (orow4 < dims.m) {
      score_store(cbase + orow4 * n4 + ocq1, acc4_1);
    }
    if (orow5 < dims.m) {
      score_store(cbase + orow5 * n4 + ocq1, acc5_1);
    }
    if (orow6 < dims.m) {
      score_store(cbase + orow6 * n4 + ocq1, acc6_1);
    }
    if (orow7 < dims.m) {
      score_store(cbase + orow7 * n4 + ocq1, acc7_1);
    }
  }
}
