...
`embed_tokens_per_layer` は `[262144, 35×256]` の 1 枚表で、f32 実体は 9.4GB —
**WebGPU の 1 バッファ上限を単独で超える**（i8 格納でも 2.19GiB）。そこで層別 35 本の
`nn.Embedding`（各 `[262144, 256]`・列スライス）へ割り、`stack` で `[1,T,35,256]` を組み直して
`per_layer_inputs=` として上流へ渡す（`input_ids` と `per_layer_inputs` の同時指定は上流が
拒否するので、`inputs_embeds` も自前で引く）。文脈射影と combine
（`project_per_layer_inputs`）は**上流実装をそのまま**通す。

MUST: 分割が上流 `get_per_layer_inputs` と**ビット一致**することを export 前に 1 回検査する
（{@link assert_per_layer_split}）。列の割り付けを間違えても形と型は合うので、golden の
突合まで誰にも見えない。検査の参照側は「チェックポイントの数行だけを載せたモデル本体の
`embed_tokens_per_layer`」で、9.4GB の f32 表をメモリに二重に持たないための形
（{@link PLE_PROBE_ROWS}）。
...
class Gemma4Wrapper(nn.Module):
    ...
    def __init__(self, model: nn.Module, tables: nn.ModuleList) -> None:
        ...
        self.per_layer_scale = per_layer_scale(model.config)
    ...
    def forward(self, input_ids: torch.Tensor) -> torch.Tensor:
        ...
        stacked = per_layer_inputs(self.per_layer, input_ids, self.per_layer_scale)
        return self.model(
...
def _per_layer_table_shape(model_file: Path) -> tuple[int, int]:
    """PLE 1 枚表の `[行, 列]`（実体は読まない）。"""
    with safe_open(str(model_file), framework="pt") as handle:
        rows, packed = handle.get_slice(PLE_CHECKPOINT_KEY).get_shape()
    return int(rows), int(packed)


def load_per_layer_tables(model_file: Path, layers: int, dim: int) -> nn.ModuleList:
    ...
    """
    rows, packed = _per_layer_table_shape(model_file)
    ...
    with safe_open(str(model_file), framework="pt") as handle:
        packed_table = handle.get_slice(PLE_CHECKPOINT_KEY)
...
def load_wrapper(model_dir: Path) -> Gemma4Wrapper:
    ...
    rows, _ = _per_layer_table_shape(model_file)
