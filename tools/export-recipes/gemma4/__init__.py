...
{@link gemma4.export_product}）と、トークナイザ資産の compile
（{@link gemma4.tokenizer} — ADR 0084 決定 1。グラフには触らず「文字列 ⇄ id 列」だけを扱う）。
...
export 台本 4 本が共有する裏方が 2 つ: PLE 1 枚表の 35 分割（{@link gemma4.ple}）と、golden を
採らない系列の出所記録（{@link gemma4.provenance}）。どちらも「同じ規律を 2 箇所に書かない」
ための分離で、台本を跨いで効く不変条件を持つものだけがここに増える。トークナイザ側の同じ役は
