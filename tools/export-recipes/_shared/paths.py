...
系列の置き場は**書き手（台本）と読み手（dist ドライバ）の共有知識**なので、綴りは
ここ 1 箇所だけが持つ。

NOTE: repo topology そのものなので core（PyPI `karume`）ではなく recipe 側にある
（ADR 0065 Consequences）— core の API は Path を受け取る形を保ち、綴りはここが注入する。
"""
...
#: リポジトリのルート（_shared/paths.py → _shared → tools/export-recipes → tools → repo）。
