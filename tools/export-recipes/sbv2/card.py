...
SBV2_JVNV_PROFILE = Sbv2CardProfile(
    ...
    attribution=(
        ...
        " quantized**",
        "  to `i8` (`i4` for the text encoder's linear weights),",
        ...
        "  of the weights — the voices in `f16` / `i8`, the text encoder in `i8` or in a mixed",
        "  form that stores its linear weights as `i4` and everything else as `i8` (the quant",
        "  table below says which storage each quant selects). No retraining, no fine-tuning —",
        "  the voices are the source checkpoints in a different storage form.",
        f"- **Text encoder**: [{SBV2_TEXT_ENCODER_MODEL}]"
        f"(https://huggingface.co/{SBV2_TEXT_ENCODER_MODEL}),",
        f"  licensed **{SBV2_TEXT_ENCODER_LICENSE}** (as of retrieval). It is redistributed here",
