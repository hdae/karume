...
SBV2_JVNV_PROFILE = Sbv2CardProfile(
    ...
    attribution=(
        ...
        "  where the mixed form stores linear weights as `i4` and everything else as `i8` (the",
        "  quant table below says which storage each quant selects). No retraining, no",
        "  fine-tuning — the voices are the source checkpoints in a different storage form.",
        f"- **Text encoder**: [{SBV2_TEXT_ENCODER_MODEL}]"
        f"(https://huggingface.co/{SBV2_TEXT_ENCODER_MODEL}),",
        f"  licensed **{SBV2_TEXT_ENCODER_LICENSE}** (as of retrieval). It is redistributed here",
