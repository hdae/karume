...
- **ビット同一ではない（式は同値・丸めだけ違う）— 実測値は下記**
  ③ DPT reassemble の `nn.ConvTranspose2d`（kernel == stride == 4 / 2・padding 0）を
     ...
     `tests/test_patch_depth_anything.py` が整数値データの `torch.equal` で固定する）。
...
def assert_supported(model: nn.Module) -> None:
    ...
    （`patch_birefnet.assert_supported` と同じ流儀）。
    """
    config = model.config
...
def _feature_fusion_stage_forward(
    ...
    （その土台は `tests/test_patch_depth_anything.py` が直に実測する）。
...
def apply_layout_patches(model: nn.Module) -> dict[str, int]:
    ...
    この段だけならパッチ前 eager と**ビット同一**で、`export_depth_anything.py --verify` の
