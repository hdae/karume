...
`export_deberta.py` と同じ役割（実重み・実トークン列での数値一致）を、SentenceTransformer 形の
**単一ベクトル出力**モデルで受け持つ。生成物は `outputs/series/` 配下で、リポジトリ直下の
`.gitignore` によりコミット対象外（重み 1.2GB 級）。

    uv run --with 'transformers==5.14.1' python export_embeddinggemma.py

transformers は **5.14.1 でピン**する（`export_deberta.py` と同じ理由 — モデリングコードが
