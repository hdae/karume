...
今回のスコープは**テキスト条件エンコーダ**（recon の G1 / G1a / G1b）だけで、DiT や codec
（G2〜G7）は後続の波でこの台本にターゲットとして足す。
...
## 何をグラフに載せるか（3 ターゲット・B=1・T は記号次元）

| ターゲット     | 入力        | 出力        | 中身                                     |
| -------------- | ----------- | ----------- | ---------------------------------------- |
| `backbone`     | `[1,T]` ids | `[1,T,768]` | ModernBERT-ja-310m（25 層・text/caption 共有） |
...
TARGETS = (TARGET_BACKBONE, TARGET_TEXT_PROJ, TARGET_CAPTION_PROJ)
...
class IrodoriSource:
    ...
    def __init__(self, source_dir: Path) -> None:
        ...
        from irodori_tts.model import PretrainedConditionProjector, PretrainedTextBackbone
...
def _pristine_outputs(
    ...
    outputs: dict[str, dict[str, torch.Tensor]] = {target: {} for target in TARGETS}
...
def export_series(
    ...
    *,
    ...
    for target, module in graphs.items():
        equivalence[target] = max(
            _check_wrapper_equivalence(
                module,
                tuple(graph_args[target][name].values()),
                pristine[target][name],
                f"{target}/{name}",
                EAGER_EQUIV_ATOL,
            )
            for name, _kind, _ids in cases
        )

    # ---- ④ export と golden の書き出し ----
    seq = Dim("T", min=2, max=sym_max)
    longest = max(cases, key=lambda case: case[2].shape[1])[0]
    written: dict[str, Any] = {}
    for target in targets:
        target_dir = out_dir / target
        target_dir.mkdir(parents=True, exist_ok=True)
        example = tuple(graph_args[target][longest].values())
        graph = export_to_file(
            graphs[target],
            example,
            target_dir / MODEL_FILE,
            dynamic_shapes=tuple({1: seq} for _ in example),
            preserved=PRESERVED_OP_PREFIXES_WITH_ATTENTION,
        )
        io_files = _write_io(graph, graph_args[target], pristine[target], target_dir)
        written[target] = {
