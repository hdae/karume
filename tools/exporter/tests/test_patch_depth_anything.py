"""`patch_depth_anything` の書き換えが原実装と同値であることの回帰テスト（実重み不要分）。

実重み 518² での端から端までの同値は export 台本（`export_depth_anything.py --verify`）の
責務で、ここは「どの書き換えがどの強さで同値か」を単体で固定する。

主張の強さは 2 段（`patch_depth_anything` の docstring と対応）:
...
from karume.patch_depth_anything import SubPixelUpsample
