"""`export_depth_anything.py` の台本レベルの約束事（実重み不要分）。

実重みを要する部分（IR の中身・パッチ前後の同値）は台本の実行と
`test_patch_depth_anything.py` が受け持つ。ここで固定するのは、**実重みが無くても壊れうる**
約束:

- 系列の綴り（`outputs/series/<モデル名>/`）が `--model-dir` に追随する
- golden 4 ケースの性格（正規化済み・互いに違う・正方）
- sanity（{@link export_depth_anything._sanity}）が**恒真でない** — 一様な出力・入力非依存の
  出力・幾何を追えていない出力を実際に落とす
- 実画像の sanity（{@link export_depth_anything._real_sanity}）も同じく恒真でない — 近い領域と
  遠い領域が並ぶ / 逆転する出力を実際に落とす
- 前処理の正本の同定（{@link export_depth_anything.check_processor}）— **resample 3 =
  BICUBIC** をはじめ、1 欄ずらすだけで落ちること
- golden io の書き出し（{@link export_depth_anything._write_io}）— **tiny な合成重み**を
...
import export_depth_anything as da
...
class TinyDepth(nn.Module):
    """{@link export_depth_anything.DepthMap} の最小の骨格（`[B,3,S,S] → [B,S,S]`）。"""
