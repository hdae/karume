...
NOTE: Anima / SBV2 の配布 recipe は wheel の外へ出た（ADR 0065 段 3+4）ので、その依存ケースは
`tools/export-recipes/<family>/tests/test_distribution.py` に居る。ここに残るのは core だけで
観測できる層（合成計画で足りる規模上限・quant 完全写像・staging/swap の不変条件・帰属
...
class TestModelName:
    ...
    def test_it_accepts_the_names_the_distributions_actually_use(self) -> None:
        for name in (IRODORI_DEFAULT_MODEL, SIGLIP2_DEFAULT_MODEL, "jvnv-F1"):
...
class TestCli:
    ...
    def test_a_caller_supplied_registry_carries_its_own_default(self) -> None:
        ...
        args = build_parser(PIPELINES, "irodori").parse_args([])
        assert args.pipeline == "irodori"
    ...
    def test_the_default_output_directory_follows_the_single_model(self) -> None:
        assert default_out_dir(PIPELINES["irodori"], ["v4-small"]).name == "karume-irodori-v4-small"
    ...
    def test_it_refuses_to_invent_a_family_repository_name(self) -> None:
        ...
        with pytest.raises(DistError, match="--out"):
            default_out_dir(PIPELINES["irodori"], ["F1", "F2"])
