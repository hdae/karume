...
- 系列名の綴りと正規化定数が**配布形（`karume.dist`）と一致**すること（上流に機械可読な
  前処理 config が無く両側が宣言を持つので、独立に動くと golden と利用者の前処理がずれる）
"""
...
class TestSeriesLayout:
    ...
    def test_the_distribution_looks_for_the_same_series_name(self):
        ...
        `karume dist` は「モデル名 → 上流リポ名 → 系列ディレクトリ名」で系列を探す。式が
        片方だけ動くと、組み立ては「系列が無い」で落ちる（それ自体は安全）が、**別の
        モデルの系列を掴む**綴りにずれた場合は誰も気づけない。
        """
        from karume.dist import BIREFNET_RESOLUTION, birefnet_series_name
        from karume.modelcard import BIREFNET_UPSTREAM
...
class TestPreprocessing:
    ...
    def test_the_distribution_declares_the_same_constants(self):
        ...
        組み立て（`karume.dist`）の両方が宣言を持たざるを得ない — 2 表が独立に動くと、
        **golden は片方の統計で焼かれ、利用者はもう片方で前処理する**形が黙って作れる。
        """
        from karume.dist import BIREFNET_IMAGE_MEAN, BIREFNET_IMAGE_STD
