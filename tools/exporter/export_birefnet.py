...
（重み + 焼いた定数で 1024² のとき 964MB）。
    ...
    uv run --group birefnet python export_birefnet.py
    uv run --group birefnet python export_birefnet.py --verify  # パッチ前 eager との同値検証

transformers は **5.14.1 でピン**する（`karume.patch_birefnet` が `trust_remote_code` の
...
定数として焼かれるので、同じ席へ 2 解像度を書くと先の資産が黙って上書きされる。
    ...
    uv run --group birefnet python export_birefnet.py --resolution 2048

モデル軸で受けているのは 2 つ（帰属表の正本は `karume.modelcard.BIREFNET_UPSTREAM`）:
...
- `inputs/birefnet/lucida/` — 上流 `egeorcun/lucida`（MIT）。BiRefNet_HR の fine-tune で、
  ...
  したがって `patch_birefnet` の差し替えも `--verify` の 2 点評価もそのまま通る。

      uv run --group birefnet python export_birefnet.py --model-dir <リポの inputs>/birefnet/lucida
...
batch は **静的 1**（`patch_birefnet` の窓マスク加算が B = 1 でのみ元実装と同値）。動的軸は
無い（`symbol_names=()`）。

MUST: モデルは `karume.patch_birefnet` のパッチを当ててから export する。当てないと Swin の
rank-6 reshape / `roll` / step-2 slice などで変換段が落ちる（差し替えの一覧は
`patch_birefnet` の docstring）。golden の期待値は**差し替え後**のモジュールから採る —
差し替え前後の差はパッチ層の責務で、`--verify` が 2 点評価で実測する。

MUST: `apply` の後に `patch_birefnet.prepare(...)` を通す（{@link load_wrapper}）。
...
（{@link REAL_CASES}）を通した golden も書く:
    ...
    uv run --group birefnet python export_birefnet.py --real-images
...
def load_model(model_dir: Path) -> nn.Module:
    ...
    patch_birefnet.assert_supported(model)
    return model
...
def load_wrapper(model_dir: Path, resolution: int) -> MatteLogits:
    ...
    MUST: 差し替えも {@link patch_birefnet.prepare} も golden を採る**前**。後に当てると
    期待値だけが元の経路で計算され、グラフと食い違ったまま緑になる。
    """
    wrapper = MatteLogits(load_model(model_dir)).eval()
    patch_birefnet.apply(wrapper.model)
    example = build_cases(resolution)[0][1]
    baked = patch_birefnet.prepare(wrapper, example)
...
def verify_patches(model_dir: Path, resolution: int) -> list[dict[str, Any]]:
    """パッチ前 eager との同値を **2 点**で実測する（`export_siglip2.py --verify` と同じ形）。
