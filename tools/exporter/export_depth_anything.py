...
生成物は `outputs/series/` 配下で、リポジトリ直下の `.gitignore` によりコミット対象外。
    ...
    uv run --group depth-anything python export_depth_anything.py
    uv run --group depth-anything python export_depth_anything.py --verify  # パッチ前との同値検証
    uv run --group depth-anything-preprocess python export_depth_anything.py --real-images

transformers は **5.14.1 でピン**する（`karume.patch_depth_anything` がモデリングコードの
...
`aten.upsample_bicubic2d`（IR 語彙に無い）が生える。`patch_depth_anything` の ② が条件を
...
MUST: モデルは `karume.patch_depth_anything` のパッチを当ててから export する。当てないと
DPT reassemble の `nn.ConvTranspose2d`（`aten.convolution.default` 2 本）で変換段が落ちる
（差し替えの一覧は `patch_depth_anything` の docstring）。golden の期待値は**差し替え後**の
...
from karume import patch_depth_anything
...
def check_processor(processor: Any) -> dict[str, Any]:
    ...
    書かずに落とす（`siglip2_preprocess.check_processor_shape` と同じ規律）。とくに
...
def load_wrapper(model_dir: Path) -> tuple[DepthMap, int]:
    ...
    resolution = patch_depth_anything.pretrained_resolution(wrapper.model)
    patch_depth_anything.apply(wrapper.model)
...
def export_series(
    ...
    written, depths = _write_io(wrapper, graph, cases, out_dir, metadata)
    summary = {
        "dir": str(out_dir),
...
def verify_patches(model_dir: Path) -> list[dict[str, Any]]:
    """パッチ前 eager との同値を **2 点**で実測する（`export_birefnet.py --verify` と同じ形）。
    ...
    """
    if patch_depth_anything.patches_applied():
        raise SystemExit(
    ...
    patch_depth_anything.apply_layout_patches(wrapper.model)
    ...
    patch_depth_anything.apply_module_patches(wrapper.model)
