...
| `--gate-strong <number>`         | library default: `0.15`                                | Self-funding gate: the "strong loss" margin. A block whose ratio exceeds `leave + strong` leaves speculation without waiting for `confirm` consecutive blocks, and an exploration burst is cut short from its `burstMin`-th cycle. **Affects the `auto` mode only** — `always` builds no gate |
...
| `--gate-explore-base <int>`      | library default: `16`                                  | Self-funding gate: the base exploration interval — in steady `speculate` the number of cycles between the plain steps that keep `W1` fresh, and in steady `plain` the initial backoff. `auto` only — must not exceed `exploreMax` (256 by default)                                            |
...
preceded the current defaults — is `--gate-strong 10 --gate-burst-min 8 --gate-explore-base 8`: the
A/B to run against the defaults on the same script.
