...
- **モデル横断の追加調査（2026-09-10〜11）**: Qwen3-0.6B / MiniCPM5-2B の RTN / GPTQ と
  ...
  [添付参照資料の現行再検証](research/2026-09-14-reference-rope-optimization.md)からK-41のpermuteコピー削減を実装。M2の20生成は検収済み。前回レビュー後の追加差分確認は残る。[K-42のattention融合](research/2026-09-15-attention-fusion.md)を任意指定で追加し、M2は画面初期選択の80生成で比較する。次は利用者依頼による過去の保留候補（K-40など）の組合せとgate/up入力共有の費用調査（数値順を変える実装は任意指定の方針を維持）。
