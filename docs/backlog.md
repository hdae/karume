...
- **モデル横断の追加調査（2026-09-10〜11）**: Qwen3-0.6B / MiniCPM5-2B の RTN / GPTQ と
  ...
  [添付参照資料の現行再検証](research/2026-09-14-reference-rope-optimization.md)からK-41のpermuteコピー削減を実装。M2の20生成は検収済み。前回レビュー後の追加差分確認は残る。[K-42のattention融合](research/2026-09-15-attention-fusion.md)を任意指定で追加し、M2は画面初期選択の80生成で比較する。[K-43の保留候補併用](research/2026-09-15-held-combinations.md)は920生成・GPU帰属まで完了。最大併用は採用せず、QATのlinear→SRQを次の製品化単位にする。常駐scaleの借用、丸め障壁、元のSRQとの比較、MTP行0一致を検収した任意指定として追加する。試作は未統合。M2のattention追試を先に行い、gate/up入力共有はこの融合の検収後に費用を測る。
