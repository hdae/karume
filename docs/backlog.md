...
- **J-4a: anima の i4 席（J-4 から切り離して先行 — 2026-08-21 ユーザー裁定）**: 第 1 段
  ...
  **取得量 −21.2%（3.48 → 2.74GB）・DiT は既定の約 2.0 倍遅い**（i4 常駐は linear の i8a8
  経路に乗れない）。**残: ①GPTQ 結線（校正コーパスを新設し評価入力と分離 — 2026-08-21 裁定
  a 案。現状の校正入力は評価プロンプトと逐語同一）②席の位置づけ（opt-in / 既定据え置き）
  ③w4a8 起票（perf-ledger Q-8）④公開の可否（HF 追加 → pin 更新 → JSR bump の連鎖）—
  いずれもユーザー裁定待ち**。irodori の席は J-4 のまま（kmeans 圧勝で別設計）。
