...
>
> **再改訂（2026-08-30 — ADR [0081](0081-shard-spec-v2.md)）**: 「1GiB 以下は単一ファイル」は
> 撤回した。配布形は**常に**「グラフ shard（`karume_ir` だけ・データ節空）+ weight shard 列」で、
> `shards` は 2 要素以上になる。本 ADR のスキーマ（決定 2 の順序付き FileRef 列・1024 上限）は
> 無改変で、先頭 = graph shard の意味づけがより強い形（実重みを持たない）になっただけ。
