...
## 追記（2026-09-10・自己採算ゲート v3 — 決定 8 の更新）

- 段 4-B ⑨ の内訳（research 2026-09-09 §6.4.1）で、負ける課題（M2 自由文・比 1.27）の損は**探索バースト 62%**・抜けるまでの
  32 cycle 36% だった。決定 8 の機構に 3 つを足す（`speculation-gate.ts`・公開の形は不変）:
  **強い負けの早抜け** `strong`（既定 0.15）— ブロック 1 本の比が `leave + strong` = 1.16 を超えたら `confirm` の連続を
  待たず plain へ倒す（勝つ文脈で誤発火する確率 ≈ 0.03%・損益分岐なら 4% だが損は 0）。**バーストの早期打ち切り**
  `burstMin`（既定 `min(4, burst)`）— バースト中、4 本目以降で比が同じ閾値を超えたら残りを回さず外れとして畳む
  （backoff 倍・`switches` は増えない・戻る側の判定精度は満ちたバーストのまま）。**探索間隔の既定 8 → 16**（バーストの
  初期間隔と `W1` の測り直しの両方）。
- 見込み（sim・受理ヒストグラムから 20,000 ターン）: M2 自由文 0.878 → 0.921×・対話 0.951 → 0.960×・RTX の勝つ側は
  0.999×。RTX の実走（research §6.5）: 勝つ 3 条件は 18 ターンで誤退出 0・W1 プローブ半減・auto / always は 0.98〜1.03。
  M2 の実走は待ち。
- bench の warm（同じ発話の追記）は model が前の答えを写して受理率が跳ねるので、ゲートが暖まる効果の検収には使えない
...
- 早抜けの利得は「抜けるまで」1 ブロックぶんで sequence の寿命に 1 回、害は勝つ課題の毎ターンに出る。よって
  **早抜けは既定 off**（`earlyLeave?`・渡した席だけ効く）とし、plain 側は `burstAbort`（既定 0.15・`burstMin` 4 本目
  以降で畳む）として残す。`exploreBase` 16 は据え置き。`exploreMax` は 256 → **512**（同日の裁定 — warm の実測で残る 2〜3% が backoff 上限で来るバースト 1 回 / ターンだったため。バーストは 2〜3 ターンに 1 回になり、投機が勝ち始めたときの反応は RTX 15 s / M2 30 s）。
