...
[利用者の80生成](../research/2026-09-15-linear-static-quantize-fusion.md#利用者のm2-attention結果)は全て出力一致した。
通常E2Bは−0.88%、QAT E2Bは−0.16%で、今回のM2では高速化の根拠にならない。
任意指定を維持しモデル既定へは昇格しない。次候補の画面では従来parallelを基準にする。
