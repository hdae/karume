...
## 追記 4（2026-09-20）: 追記 3 の撤回と、並列 GEMV 族の積和を明示 `fma()` で綴る決定

追記 3 の丸め障壁は M2 で効かなかった（同じ組が 11 → 12 要素の不一致・件数が動いただけ）。
原因を式形の再結合と読んだのが誤りで、撤回する。障壁と `rounding_mask` の語は外した。

診断スクリプト `tools/diag/packed-activation-probe.ts`（生成 WGSL の字面を変種ごとに書き換えて同じ
入力で回し、f32 経路の出力と u32 で突き合わせる）を M2 と RTX で回した結果:

| 変種                                        | RTX（Vulkan） | M2（Metal・wgpu-hal） |
| ------------------------------------------- | ------------: | --------------------- |
| f32 + 活性ロードの XOR 障壁（数値上は恒等） |             0 | 5〜22 要素が割れる    |
| f32 + 明示 `fma()`                          |             0 | 12〜21 要素が割れる   |
| packed（障壁あり / なし）                   |             0 | 12〜28 要素が割れる   |
| packed + 積ごとの XOR 障壁                  |   4〜1536 ULP | 割れる                |
| **f32 + fma と packed + fma の相互一致**    |          一致 | **4 組とも一致**      |

読み: RTX の f32 経路はコンパイラが積和を fma に縮約した結果で、明示 fma と u32 同一（540 組の
掃引でも不一致 0）。Metal は**同じ数式でも式形ごとに縮約の入れ方を変える**（恒等の書き換えで
f32 自身が割れる）ので、式を似せても 2 本のカーネルは揃わない。明示 `fma()` は縮約の自由度そのものを
消すので、両経路が同じ丸めになる。

### 追記決定 6: 並列族の積和は `acc = fma(x, d, acc)`

- 対象は `parallelWgsl` が生成する全変種（f32 / packed × linear→SRQ 融合なし / あり）。逐次 GEMV・
  行ブロックは参照経路の数値を動かさないため従来の綴りのまま。subgroup32 変種は 2026-09-25 に同じ
  綴りへ揃えた（ADR 0101 追記 — parallel と同じ和を出す変種なので「参照経路を動かさない」の除外に当たらない）。
- RTX では数値・id 列とも不変（掃引 540 組・QAT E2B 64 / 256 token の生成文が変更前と同一）。
- **Metal では並列経路（`i4-fast` / `i4-gemvpar`）の出力が変更前の値から変わる**。変更前の値は
  コンパイラの縮約の癖で決まっていたもので、変更後は packed on / off・融合の有無で一致する状態になる。
  参照席 `i4`（逐次）は不変。
- 決定 3 の「乗算 1 個は正しく丸めた積」はそのまま正しい。割れていたのは復元でなく積和の縮約。

検収（RTX）: u32 一致門 540 件・融合変種・並列 GEMV の門・QAT E2B の id 列一致（on / off）が緑。
