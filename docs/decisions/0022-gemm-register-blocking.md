...
> 追記（2026-08-10・同日後刻）: **f32/f16 骨格もタイル幾何をパラメタ化した**
> （`src/kernels/gemm-geometry.ts`・既定は全 op 共通 r8×4 wg16×8・出力タイル 64×64 と
> K タイル 16 は不変・幾何はパイプラインキーに載る）— 直前の「そちらの幾何は動いていない」は
> ここで失効。決定 3 の MUST 自体は不変: 幾何が変えてよいのは**担当割りだけ**で、1 出力要素
> あたりの K 縮約順（外側 t / 内側 kk とも昇順）と積和式の字面は幾何によらず同一。ただし
> i8a8 と違い f32 縮約に順序非依存の理論保証は無く、ビット同一は**実測命題**（幾何 3 変種 ×
> PNG 門 4 本 + WAV 門で sha256 不一致 0・fma 縮約の兆候なし —
> [research/2026-08-10-f32-geometry-probe.md](../research/2026-08-10-f32-geometry-probe.md)）。
> MUST: 幾何の既定変更は門の再実測とセット。**実行時に幾何が変わる形（オートチューン）は
> f32/f16 では取らない**。副産物として accumulator の静的展開（帰結 2 段目の予言）を全 op へ
> 横展開 — これ単独で f16-1024 ×1.15（幾何の上積みは従）。合計 f16-1024 30.4→23.7s
> （×1.28）・w8a8-s16-1024 11.7→10.7s（×1.09 — VAE conv2d と attention f32 が骨格を共有）。

> 追記（2026-08-10・タイル軸掃引）: タイル辺も幾何へ吸収し（tileM = regM·wgY /
> tileN = regN·wgX の導出・dispatch とキーは生成と同一の解決点を通る）、掃引の結果
> **既定を `M128N128 r8×8 wg16×16` へ変更**（f16-1024 23.7→20.2s ×1.17・w8a8 中立・
> PNG/WAV 全門再実測でビット同一 —
> [research/2026-08-10-f32-geometry-probe.md](../research/2026-08-10-f32-geometry-probe.md)
> §タイル軸掃引）。conv2d だけ m タイル 64/32 のヒューリスティックを維持（n は 128 追随）。
> 最良タイル形は i8a8 の M128N64 と一致せず、**幾何の既定は経路（数値契約）ごとの実測で
> しか決めない**ことが追認された。共有 16 KB・threads 256 は WebGPU 既定上限ちょうど。
> Metal（Apple M2）は同日追試済み: **退行なし**（f16 ×1.06・w8a8 ×1.04）で spill 懸念は
> 否定され、幾何 2 種の出力 sha が全系列で一致 — ビット同一の実測データ点は Vulkan / Metal
> の 2 環境になった（research 同 §Apple M2）。DECIDED: 既定はバックエンド共通の 1 本
> （NVIDIA 第一 — ユーザー裁定 2026-08-10）。バックエンド別の静的既定は、実測で明確な退行が
> 出た場合にのみ検討する（その場合も実行時オートチューン禁止は不変）。

> 追記（2026-08-11・skinny-M バケット）: 幾何の選択を**行数 M の静的バケット**へ拡張
> （`gemmGeometryForRows` — matmul / bmm / linear の 3 経路のみ。融合 attention ①③・conv2d・
> i8a8 は不変）。既定 M128N128 は M < 128 で行タイルが 1 枚しか立たず、M=T の小さい系列
> （EmbeddingGemma bare T=4: linear 170 本 = GPU の 87%）で occupancy 律速になるため。掃引
> （候補 11 種 × 実形状 × M∈{1,4,64,318,512}・全点ビット同一 —
> [research/2026-08-11-skinny-m-geometry.md](../research/2026-08-11-skinny-m-geometry.md)）で
> **M ≤ 64 → `M16N16 r1×4 wg4×16`（対既定 ×3.0〜3.2）・M ≥ 65 → 既定のまま**に確定。選択は
> プラン時 shape の純関数でキーに幾何判別子が載る — 実行時オートチューン禁止・「既定変更は
> 門の再実測とセット」の MUST はそのまま（PNG/WAV 門再実測済み・sha256 全一致）。ビット同一の
