...
- **定義域は 0 本を含む**（第 3 巡で追加）: 値を定義しない effect op（ADR 0067 の
  ...
  **0 本の解禁は当時の `docs/ir-v1.md`（現 [docs/ir-v2.md](../ir-v2.md) の前身・`2960ce37^` の版）の改訂を要する**（本文が outs ≥ 1 を明記している — 実装波で
...
## 追記 10（2026-09-12）: 非正規数を失わない最大値・上位候補の比較

- `argmax` と `topk` の値の比較を整数の順位キーで行う。入力の f32 ビット列から、
  正の値は符号ビットを反転、負の値は全ビットを反転した順序へ写す。
  ±0 は同じキー、NaN は全て最大キーとする。キーが同じなら従来どおり最小 index を選ぶ。
- これは数値契約の緩和ではなく、非正規数をゼロと区別できなかった実装の修正である。
  [WGSL §15.7.2](https://www.w3.org/TR/WGSL/#floating-point-evaluation) は浮動小数点比較などで
  非正規数のゼロ化を許しており、読み出し後の値が元のままでも比較結果は変わりうる。
  [再現・検収と性能](../research/2026-09-10-codex-mtp-optimization.md#最大値選択の非正規数対応2026-09-12)を記録した。
- 変換は入力の走査時に一度だけ行う。入力・scratch・部分結果は u32 のまま扱い、
  途中の f32 bitcast も避ける。部分結果の 2 語は順位キーと index になる。
  値を返す topk は、選ばれた index の元のビット列を読み直し、NaN payload と符号付きゼロも保つ。
- 通常・分割の argmax / topk の生成キーを v2 にする。dispatch の形と uniform のレイアウト、
  workgroup storage 由来の k 上限は変えない。公開 API・IR・保存資産・出力の dtype は維持する。
  行列計算や他の演算の丸め方・許容誤差は変更しない。
- 比較時に毎回ビット順を変換する案も検証した。連続投入の単体比較で優位だった入力時だけの変換を採用する。
  回帰テストは CPU の通常の数値比較を基準とし、GPU の順位変換を複製しない。
  非正規数・NaN・±0・Infinity・同点・分割境界・大語彙・一般 k を検証する。
  同点方向、有限 sentinel、NaN 優先順位の故障注入を維持する。
