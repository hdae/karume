...
## 追記（2026-09-10 — 記号依存マスクのガード）

記号長 5 / 9 でマスクの各行が非空でも、それ以外の長さで非空とは限らない。
`arange(T) >= 4` は T=2..4 で全マスクとなる反例である。決定 2 の不活性判定は
記号依存のマスクを証明成功にしない。既存の `safe_softmax` への置換で意味論を保つ。
静的マスクの除去と、非有限値の源がない経路の除去は維持する。
