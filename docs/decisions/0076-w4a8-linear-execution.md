...
## 追記（2026-08-31 — OP 数値レビュー G1-1）

§2 の丸め回数の勘定（w8a8 = 2 回 / w4a8 = `k/g + 1` 回）は **fma を単一丸めとして数えた**
ものだが、WGSL 仕様は `fma(x,y,z)` の精度を `x*y+z` から**継承**と規定するのみで、真の融合
（単一丸め）を要求しない（§15.7.4.1・非融合展開の実装も適合と §17.5.32 Note が明記）。
したがってこの勘定と CPU 参照との atol=0 一致は**仕様の帰結ではなく融合実装での性質**であり、
非融合実装では 1 ULP 帯の差が仕様適合のまま生じ得る（Metal の 1 ULP 差 = known-issues が
