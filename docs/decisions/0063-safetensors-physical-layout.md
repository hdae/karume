...
上書きは発効した。重みの配布形としての書き出し順の契約は使われない。safetensors の読み手
（`packages/runtime/src/format/safetensors.ts`）は付帯資産を読むために残り、データ節の被覆と要素整列の
検査はそちらに掛かり続ける。量子化の方言 dtype `I4` / `I2` は受理しない。
