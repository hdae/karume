...
## 追記 2026-09-05 — 同一 checkpoint の 2 解像度を同居させたリポで、2048 側の shard 1 本が回復しない

時点記録（hf_xet 1.6.0・停止ノブ 4 本・shard-cache 退避・台本 `tools/release/hf-upload.zsh`）。
対象は初公開の `karume-birefnet-hr` / `karume-lucida`（各 1024² + 2048² の 2 モデル = 5 + 6 shard）。

| 上げ方                                                       |      1024 の shard 2 |                  2048 の shard 2 | それ以外の 9 本 |
| ------------------------------------------------------------ | -------------------: | -------------------------------: | --------------: |
| ① リポ丸ごと 1 セッション                                    | 19.4 / 10.1 MiB/term |                    **1.4 / 0.5** |          19〜45 |
| ② 削除 → 再作成し 1024 側 → 2048 側を別セッション            |            8.3 / 6.8 | **0.5 / 0.5**（terms 472 / 454） |          24〜62 |
| ③ ②の後、2048 の shard 2 だけ delete → 再 up（2 コミット法） |                    — | **0.6**（terms 390・6 秒で終了） |               — |

（左が birefnet-hr・右が lucida。global dedup は 3 回とも `query_dedup=0` で止まっていた。）

- **2 コミット法は効かない**（③）: 同一バイトの再 up は転送がほぼ起きずに終わり、reconstruction は
  元の xorb を参照したまま。§3 / §5 の「片道ラチェット」は同一リポ内では 1.6.0 でも残る。
- **削除 → 再作成でも 2048 の shard 2 は直らない**（②）: セッション内 dedup（1024 側の xorb への参照）
  を疑って別セッションに分けたが、terms はむしろ増えた。reconstruction が参照する xorb は 2 本だけで
  終端が 472 回入れ替わる形（1 xorb 64 MiB 上限の 223 MiB ファイルとしては説明がつかない）。
  ファイル内の同一内容テンソルは 68 本中 6 本 9.8 MiB（相対位置表の類）で、内容だけでも説明が
  つかない。**アップローダ側の xorb 組み立て（並列書き）の癖**を疑うが未解明（openQuestions）。
- **DL 実測**（`curl -L -o /dev/null`・同じ回線）: 2048 の shard 2 = **4.6 MB/s（51 s）**、健全な
  shard 4 = **10.9 MB/s（22 s）** → 2.4 倍。2048 モデル 1 本の DL に対しては 1/6 の shard が遅い形。
- 裁定: **このまま公開**（正しさには影響なし・2048² は任意選択のモデル）。回復手段が見つかったら
  上げ直す（runbook §2 の回復手順はこの形には効かない旨を注記）。
