...
Mac（Apple M2 / Metal / Deno 2.9.4・ユーザー実測・各 3 回）の最終形（同じ台本・パイプライン面）:

| 構成                             | 器のみ・1GiB shard（結果 6） |         **器 + 256MiB shard** | 対 器のみ |
| -------------------------------- | ---------------------------: | ----------------------------: | --------: |
| anima f16・最大 RSS（`time -l`） |    3,068 MiB（2,974〜3,158） | **1,435 MiB（1,425〜1,441）** |      −53% |
| anima f16・サンプル最大          |             2,936〜2,940 MiB | **1,224 MiB（1,185〜1,270）** |      −58% |
