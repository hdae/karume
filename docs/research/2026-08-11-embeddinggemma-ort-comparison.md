...
## 6. 追記（同日）: 最適化 3 波後の再計測

skinny-M 幾何 + rope 融合（`29aafc6`/`12abef9`）→ 融合 attention mask（`3a6b961` 系）→
中 M バケット（`a81cc08`）の後の karume 壁時計（同一プローブ規約: warm・median of 20）。
ORT 側は §1 の同日実測を再掲（ORT 側は無変更なので再実測していない）:

| ケース (T)          | ORT Node fp32 CPU | **ORT Web WebGPU** | karume §1 時点 | **karume 3 波後** | 対 ORT GPU 比 |
| ------------------- | ----------------- | ------------------ | -------------- | ----------------- | ------------- |
| bare (12)           | **14.5**          | 21.4               | 76.4           | **53.8**          | 3.6 → **2.5** |
| query-en (16)       | **15.7**          | 20.0               | 76.4           | **53.9**          | 3.8 → **2.7** |
| query-ja (16)       | **15.8**          | 19.5               | 76.5           | **57.3**          | 3.9 → **2.9** |
| document-en (19)    | **17.6**          | 19.5               | 76.1           | **56.9**          | 3.9 → **2.9** |
| long-document (318) | 203.9             | **28.7**           | 81.1           | **63.5**          | 2.8 → **2.2** |

- 短文の残差はほぼ全てホスト固定費（GPU 実時間 ~15ms・wall 54ms — dispatch 数 −456 本でも
  wall が動かないことから per-run 固定費 ~38ms と帰属。§3a / skinny-m doc §3）。ORT Web が
  ~20ms で回ることは「Chrome + Dawn のホスト経路は同じ仕事をもっと安くこなせる」ことの
  実証でもあり、**次の対象はカーネルではなくホスト**（Deno WebGPU の per-call 費用・
  同期待ちの分解）。
- CPU 系との損益分岐は T ≈ 40〜60 へ改善（T=19 で 3.2 倍差・T=318 で 3.2 倍勝ち）。
- 数値忠実度（§2）は 3 波を通じて不変（全門ビット同一・e2e atol 1e-6 のまま）。

## 7. 追記（2026-08-16）: 0.3.0 リリース時点の再計測

常駐 run（ADR 0054・単一フェンス）と全体レビュー波（0.3.0 — batch リース等の門追加込み）後の
karume 壁時計（同一プローブ規約: warm・median of 20・同一機）。ORT 側は §1 の 08-11 実測を
再掲（JSEP 世代・未再測 — native WebGPU EP + graph capture での再測が正式比較の条件。
測定条件の規範は [2026-08-16-runtime-landscape.md](2026-08-16-runtime-landscape.md) §4）:

| ケース (T)          | ORT Web WebGPU (08-11) | karume 3 波後 (08-11) | **karume 0.3.0** | 対 ORT GPU 比  |
| ------------------- | ---------------------- | --------------------- | ---------------- | -------------- |
| bare (12)           | 21.4                   | 53.8                  | **28.11**        | 2.5 → **1.31** |
| query-en (16)       | 20.0                   | 53.9                  | **27.96**        | 2.7 → **1.40** |
| query-ja (16)       | 19.5                   | 57.3                  | **28.15**        | 2.9 → **1.44** |
| document-en (19)    | 19.5                   | 56.9                  | **28.05**        | 2.9 → **1.44** |
| long-document (318) | 28.7                   | 63.5                  | **39.94**        | 2.2 → **1.39** |

- 短文帯の改善は §6 の帰属どおり**ホスト固定費の解消**（ADR 0054 の per-run 単一フェンス化・
  EG bare 52.54 → 28.15ms）で、レビュー波の契約機構（batch リース / resident owner 検査等）を
  足しても 28.11ms と**測定誤差内で不変** — 門は性能コストゼロ。
- 残差 ~1.3〜1.4 倍もなおホスト経路の比較（GPU 実時間 ~15ms 級 — §3a）。ORT 側を
  native EP + capture で再測すると差は開き得る。次の一手はベンチ慣行（backlog later）で
  条件を揃えてから。
