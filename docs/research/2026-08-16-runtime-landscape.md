...
- 短文域は karume 側がホスト固定費支配だったが、常駐 run（ADR 0054）で解消が進み、
  **0.3.0 時点の再計測（2026-08-16・同一プローブ規約）は bare 28.11ms** — 08-11 の 54ms 級から
  半減し、対 ORT Web（08-11 の JSEP 値 ~20ms・未再測）は ×2.5 → **×1.3〜1.4**。数値の正本は
  既存 doc の §7 追記。残差もなおホスト経路の比較（GPU 実時間 ~15ms 級）で、§4.1 の
  graph capture + native EP を ORT 側に入れると差は開き得る — それを承知で条件を揃えて測る。
