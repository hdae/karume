...
| ----------------------------------- | ----------: | ----------: | ------ | ------------------ |
| **A** 末尾 2 層カット               | −25,346,048 | 309,199,288 | 低     | **波 1 で採用**    |
| **B** `const.*` 2 本をグラフ入力化  |  −2,097,152 | 332,448,184 | 低〜中 | **波 3 で採用**    |
...
> **A の実測（2026-08-11・波 1 着地 — ADR [0045](../decisions/0045-deberta-layer-trim.md)）**:
...
> **B の実測（2026-08-11・波 3 着地 — ADR 0045 決定 5）**: 309,166,896 → **307,068,768 B**
> （−2,098,128 = 表 2,097,152 + ヘッダ 976）。ノード 1,130 → 1,128、initializer 366 → 364。
> **速度は中立**（順序を入れ替えた 2 回の A/B で焼き込み 153.4 / 171.2ms に対し外部供給
> 159.5 / 164.3ms @ T=512 — 順序のばらつきの方が大きい）。表生成のホスト費用は T=512 で 1.2ms、
> 実用の T=15〜35 では 0.02ms 未満。WAV sha256 は不変。
>
> 実装コストは見積もりより重かった: 前例（flow / voice）は自前モジュールのパッチだったが、
> DeBERTa は **transformers の `disentangled_attention_bias` を丸ごとコピー改変**し、
> `HiddenStatesWrapper` も `DebertaV2Model.forward` の再実装が要った（前者は `relative_pos` を
> encoder へ渡す口を持たない）。ユーザー裁定（2026-08-11）で実装続行。
...
> **波 2 の実測（2026-08-11・着地 — ADR 0045 決定 3/4）**: 期待は**外れた**。A/B（warmup 2 +
...
（追記 2026-08-16: この欠落は `e2e_deberta_test.ts` の移植で解消 — 本文は時点記録として据え置き。）
