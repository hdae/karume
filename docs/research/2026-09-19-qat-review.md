...
## 12. 実装後の自動検証（2026-09-19・main `776923d`）

裁定に基づく修正（コミット `ec26ab3`〜`776923d`）と配布形の再ビルド後に、RTX 3080 Ti で実行した。

| 検査                                                     | 結果                                                                                                                                                                                                                                                                                                                                                                                          |
| -------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `deno task verify`（fmt / lint / check / test・実 GPU）  | 2983 passed（868 steps）/ 1 failed / 5 ignored（31 分 24 秒）。failed 1 は opbench の実資産 census が「既定 quant はノブを宣言しない」前提で `session` を `{}` と主張していたもので、再ビルドで既定が `i4-fast` になったため割れた。参照席 `i4` を名指しする形に直して単独で緑（`776923d`）。ignored 5 は既知（母音検出の実音声 4・subgroups 2 ステップ）                                     |
| `uv run pytest`（tools/exporter）                        | 3234 passed / 1 skipped（既知）                                                                                                                                                                                                                                                                                                                                                               |
| `uv run pytest`（tools/export-recipes）                  | 2838 passed / 4 skipped（既知）                                                                                                                                                                                                                                                                                                                                                               |
| QAT 再 export                                            | E2B / E4B とも `reference.json` schema 2・`maxChunkLength` 768・`upstreamUnused.kvCacheScales` E2B 70 / E4B 84                                                                                                                                                                                                                                                                                |
| 配布形の再ビルド                                         | `models/karume-gemma4-qat`: e2b 既定 `i4-fast`（`i4` / `i4-gemvpar` / `i4-fast`）・e4b `i4`。`models/karume-gemma4`: e2b 既定 `i4-fast`。両方 capacity 4096 / chunkLength 768 / maxChunkLength 768                                                                                                                                                                                            |
| QAT CLI のスモーク（同じ prompt・12 token・warmup なし） | 既定（`i4-fast`）: capacity 4096 / chunk 768 で生成・decode 26.0 tok/s。`--quant i4-fast --linear-gemv-reduce sequential --fuse-linear-static-quantize false`: 同じ 12 token・decode 20.7 tok/s。`--quant i4-fast --linear-gemv-reduce sequential` のみ: `Gemma4QatPipeline.fromPretrained: quant 'i4-fast': fuseLinearStaticQuantizeはlinearGemvReduce: parallelが必要` で拒否（設計どおり） |

未検証のまま: 512 超の文脈での出力品質（既定 capacity 4096 は到達可能にしただけで検収済みという意味ではない）、長文の速度、Chrome での `i4-fast` 実走。
