...
### §4.2 32K / 128K の再測（クリーン GPU・2026-09-03 追記）

他プロセスを落とした状態で §4.1 の 2 点を取り直した（capacity 131,072 で固定・K-12 on・
`--no-stop`・合成プロンプト）。

|       P | chunkLength |              prefill | decode 中央値 ms/token |
| ------: | ----------: | -------------------: | ---------------------: |
|  32,000 |         768 |              208.6 s |                   56.1 |
|  32,000 |         192 |              301.8 s |                   51.9 |
| 131,000 |         768 | 2,571 s（171 chunk） |                  112.4 |

- **P=131,000 は完走した**。前回の `GpuOutOfMemoryError` は環境要因（他プロセスの VRAM 占有）で確定。
  full 層の KV は 1,536 MiB（capacity 131,072 × 12,288 B/token）で、見積り（§4 の peak 4,956 MiB）どおり
  12 GB 機に載る。
- prefill の 1 chunk あたりは 19.6 ms/token（2,571 s ÷ 131,000）。P² の項が効くので P=32,000 の
  6.5 ms/token から 3 倍に伸びている。
- decode は 32K の 56.1 ms から 128K で 112.4 ms へ約 2 倍。KV 長に線形な ①QK（perf-ledger K-14）が
  支配する形は §3 の見立てどおり。
- cl 192 は cl 768 より prefill が 1.45 倍遅く、decode はわずかに速い（測定代の範囲）。
- **token 列は評価していない** — 合成プロンプト（同文の繰り返し）なので、列の一致は品質の証拠に
  ならない。この 3 点は時間と VRAM だけの記録である。
