...
## M2検収後の更新（2026-09-13 15:41 UTCの添付）

[次の調査記録](2026-09-13-gemv-subgroup.md)に追試を保存した。40生成の同方式内不一致0、
decodeは34.899→35.081 tok/s（約0.52%）、TTFTはほぼ不変。安定した利得とは判断せず、任意指定を維持する。
既定quantへは採用しない。初期選択はK-38のGEMV比較へ移す。
