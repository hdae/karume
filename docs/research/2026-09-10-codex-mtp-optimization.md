...
## M2の入力バケット追試と初期選択（2026-09-13）

[利用者の全26設定・260生成](2026-09-13-m2-prefill-adoption.md)を照合。反復と同じ加算方式でのバケット変更による不一致0。
並列時の日本語TTFTは通常約672→446 ms、QAT約805→525〜532 ms。decodeの改善とは区別する。
通常版の逐次設定で時間経過による速度変動があり、全走行を一律に同じ熱/電源条件とは扱わない。
比較画面の初期選択を両E2B・Karume・並列加算・細分化へ変更する。2設定20生成で次の候補の基準を取れる。
利用者へ追試を依頼するときは必要な比較を初期選択へ反映する方針をAGENTSへ永続化した。
標準/全パターンの選択肢、URL/API省略時、モデル・runtime・CLIの既定は維持する（ADR 0066追記）。
全体の順番は融合の候補調査へ戻す。M2の今回の260生成をそのまま取り直す依頼は不要。
...
## 大きいI4行列の配置比較を完了（2026-09-14）

[全体比較とGPU帰属](2026-09-14-i4-lane-comparison.md)、[集計JSON](2026-09-14-i4-lane-results.json)を追加（K-39）。
通常E2BのI4g32 N12288/K1536をL4→L8にした隔離候補は、80生成で112.090→100.056 tok/s、約10.7%遅く不採用。
通常passのGPU時間も約6.50→7.13 ms/tokenへ増え、対象dispatchは約27→43 µs。暖機つき単体再計測も同じ傾向だった。
36条件・483,444要素は既存許容帯と同設定のM1/4/8先頭行一致を通過。全体・帰属の計112生成はtoken/stop/text一致。
製品の設定・数値契約・資産は変更しない。前回の単体探索だけを根拠にした採用はしない。

次は利用者の依頼どおり、`codex/review-and-fix`の差分・依存関係・検証範囲を整理してレビューエージェント向け資料を準備する。
M2のGPU費用帰属とRMS融合＋投入768のモデル既定化は未完として残す。同じM2の80生成は再依頼しない。
...
## 保留候補の併用再検証と次の実装（2026-09-15）

[詳細](2026-09-15-held-combinations.md)と[結果JSON](2026-09-15-held-combinations-results.json)へ記録した。
attentionは`b9d41ef`にコミット済み。その後、過去の候補を現行コードへ載せ直して920生成、
さらに35生成のGPU診断を行った。最大併用を製品既定には採用しない。
温度を管理した比較ではQATが基準94.623 tok/s、linear→SRQ単独99.174、融合内の探索展開付き99.810、
軽い融合の併用102.958。RTX/Chromeの実測でありM2の値ではない。

次はlinear→SRQの独立した任意指定への整理。実験用のWGSL文字列置換や、
重み転置をrecipe導出中に実行する試作をそのまま移植しない。
元のSRQとの360数値条件は成功したが、製品経路・借用scale寿命・MTP・M2は別に検収する。
現在の利用者向け画面は両E2Bのattention往復80生成を維持する。全組合せを再依頼しない。
未完と順序は[backlog](../backlog.md)、マージ時の追加範囲は[レビュー資料](2026-09-14-merge-review.md)を読む。

## 実行設定の型検査を補完（2026-09-15）
