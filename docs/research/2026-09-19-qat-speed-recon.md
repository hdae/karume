...
2. ③（lm_head・逐次）だけは算術が 36% で、ここは整数内積が効く。形ごとに効くレバーが違う。
   **訂正（2026-09-20）**: 実モデルの lm_head には int8 活性が無い（公式 checkpoint の SRQ scale が 0 = 恒等で recipe は挟まない）。
   整数内積は公式にも無い動的量子化を要し、上限も 0.24 ms/token の 36% ≈ 0.09 ms なので棄却（ADR 0105 追記 2・K-52 と同じ判定）。
...
条件は §13 と同じ（QAT E2B `i4-fast`・capacity 4096 / chunk 768・64 token・greedy）。Chrome は off/on/on/off/off/on の 6 ロード × 2 prompt、
Deno は温度 0 で 64 / 256 token × 3 回。生値は [結果 JSON](2026-09-19-k45-packed-activations-results.json)。生成 id 列は全対で同一。

| 回 | 形                                                                                   | Chrome 壁（中央値の中央値 off → on） |     Chrome GPU（pass 境界） |                        Deno 64 / 256 token |
| -- | ------------------------------------------------------------------------------------ | -----------------------------------: | --------------------------: | -----------------------------------------: |
| 1  | 全 210 本の SRQ を packed・SRQ カーネルは count/4 スレッド                           |           94.5 → 94.1 tok/s（−0.5%） | 6.97 → 7.23 ms（**+0.25**） |          43.1 → 43.4 / 39.5 → 39.7（中立） |
| 2  | 実測で効いた 4 形だけ packed（70 本）・SRQ は 1 スレッド 1 要素 + 共有メモリで詰める |      **95.3 → 103.3 tok/s（+8.3%）** | **7.20 → 6.63 ms（−0.57）** | 43.5 → 44.7（+2.9%）/ 39.7 → 40.4（+1.5%） |

1 回目の per-key（dispatch 分割・補正）で分かったこと: 効くのは **K が長く lanes 32 の形だけ**（wi2:l32 = down 0.62 倍・wi4g2048:l32 0.79〜0.81・
wi4g4096:l32 0.77）。K=1536 の lanes 2 / 4 と i8 は効かないか悪化（wi2:l2 = gate/up **1.23 倍**・wi8 1.05）— 追加の unpack / 変換 / 乗算が
ロード削減を食い潰す。packed SRQ カーネルは count/4 スレッドの直列 4 要素で占有率不足（1.19 倍・+0.39 ms）。
2 回目はこの 2 点を直した結果で、GEMV 側 −0.51 ms（3 形）+ SRQ ±0 = GPU −0.57 ms。

**単体スパイク（§14）との食い違い**: §14 は gate/up（lanes 2）でも活性ロード削減が 2.7 倍と読んだが、実モデルでは 1.23 倍の悪化だった。
単体ハーネスの外挿は形ごとに実モデルで確かめる（形の採否は実測表で持つ — ADR 0022 / 0098 の流儀）。

判定: **席は有効（opt-in・既定 false のまま）**。次の手は ①`i4-fast` quant への宣言（manifest の SessionSpec に席を足す + 配布形の再ビルド）と
M2 追試（同ページの往復比較）②段 1b（整数内積）は算術が 36% を占める lm_head 形（逐次 GEMV・`PARALLEL_SHAPES` 外）に絞る
③効かなかった形（K=1536・lanes 2 / 4）の別手は保留。
...
**切り分け結果（同日・利用者実走）**: SRQ の符号列と復元 `f32(code) × scale` は M2 でも一致し、並列 GEMV の packed 変種だけが
最初の組（i2・lanes 2・非融合・M=1・11 要素）から不一致。差は「活性がロード値か積の式か」の 1 点で、Metal のコンパイラが
`(c × s) × d + acc` を再結合・縮約したと読む。対策 = 復元した quad に実行時 0 との XOR 丸め障壁（ADR 0099 と同じ書き方・
ADR 0105 追記 3）。RTX では数値不変（u32 門 540 件・id 列一致が緑のまま）。**M2 の再走 = 効かず**（11 → 12 要素）。

**機序の確定（同日・`tools/diag/packed-activation-probe.ts` を M2 と RTX で実走）**: M2 では f32 経路に数値上恒等の書き換え（活性ロードの XOR 障壁）を
入れただけで f32 自身が 5〜22 要素割れる = Metal のコンパイラは式形ごとに fma 縮約の入れ方を変える。一方 `f32+fma` と `packed+fma` は
4 組とも相互に一致し、RTX では f32 = f32+fma = packed = packed+fma（540 組の掃引で不一致 0）。対策 = 並列 GEMV 族の積和を明示
`fma()` で綴る（ADR 0105 追記 4・追記 3 は撤回）。RTX の生成文は変更前と同一・Metal では並列経路の出力が変わる（両経路は揃う）。

**M2 再検収（同日・fma 化後・checkout `04c55dd`）**: u32 一致門は 3 段とも緑。計測ページの往復
（[M2 結果 JSON](2026-09-20-m2-packed-activations-fma-results.json)）:

| ロード | packed 活性 | 英語 decode tok/s（中央値 [範囲]） | 日本語 decode tok/s | TTFT 英 / 日 |
| ------ | ----------- | ---------------------------------: | ------------------: | -----------: |
| 1      | off         |                   35.8 [32.2–35.9] |    35.7 [31.8–35.8] | 386 / 478 ms |
| 2      | on          |                   35.9 [35.3–35.9] |    35.7 [30.8–35.9] |    394 / 478 |
| 3      | on          |                   35.8 [35.4–35.9] |    33.3 [30.1–33.7] |    389 / 479 |
| 4      | off         |                   33.3 [30.8–34.7] |    31.2 [30.8–35.0] |    392 / 509 |

中央値の中央値: off 35.8 / on **35.9 tok/s**（中立）。**id 列は英語・日本語とも 4 ロード全て同一**（off / on の分岐は消えた）。
fma 化前の off / on どちらとも一致しない = Metal の並列経路の出力は変更前から変わった（予告どおり）。要判断 4 は「宣言を維持」で閉じる。
